...
WORKSPACE_ROOT = BENCHMARKS_ROOT.parent
CUPF_ROOT = WORKSPACE_ROOT / "cuPF"
if not (CUPF_ROOT / "CMakeLists.txt").exists():
    CUPF_ROOT = WORKSPACE_ROOT
...
DEFAULT_END2END_BUILD_DIR = WORKSPACE_ROOT / "build" / "bench-end2end"
DEFAULT_OPERATORS_BUILD_DIR = WORKSPACE_ROOT / "build" / "bench-operators"
...
PROFILE_SPECS: dict[str, ProfileSpec] = {
    ...
    "cuda_fp32_edge": ProfileSpec("cuda_fp32_edge", "cupf", "cuda_fp32_edge", "cuda_fp32_edge", "cuda", "fp32", "edge_based"),
...
def write_readme(run_root: Path, manifest: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        "This directory was generated by `benchmarks/run_benchmarks.py`.",
        "",
        f"- Dataset root: `{manifest['dataset_root']}`",
        f"- Measurement modes: {', '.join(manifest['measurement_modes'])}",
    ...
    for mode in modes:
        ...
        for case_name in cases:
            for profile in profile_specs:
                ...
                if args.batch_size != 1 and not (
                    profile.backend == "cuda" and profile.compute in {"mixed", "fp32"}
                ):
                    raise ValueError("--batch-size > 1 is currently supported only by CUDA mixed/fp32 profiles")
