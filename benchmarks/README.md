...
- `cpp_naive`: C++ PyPower-like Jacobian + KLU reference path.
...
- `cuda_fp32_edge`: CUDA FP32 profile with FP32 device buffers/operators and cuDSS32 solve.
...
- `cpp_pypowerlike`: reference CPU baseline using the PyPower-like Jacobian and KLU solve.
...
```bash
python3 /workspace/gpu-powerflow/benchmarks/run_benchmarks.py \
...
```bash
cmake -S /workspace/gpu-powerflow/cuPF -B /workspace/gpu-powerflow/build/bench-end2end \
...
cmake --build /workspace/gpu-powerflow/build/bench-end2end --target cupf_case_benchmark -j

/workspace/gpu-powerflow/build/bench-end2end/benchmarks/cupf_case_benchmark \
...
```bash
cmake -S /workspace/gpu-powerflow/cuPF -B /workspace/gpu-powerflow/build/bench-operators \
  -DCMAKE_BUILD_TYPE=Release \
  -DBUILD_BENCHMARKS=ON \
  -DENABLE_LOG=OFF \
  -DENABLE_TIMING=ON \
...
cmake --build /workspace/gpu-powerflow/build/bench-operators --target cupf_case_benchmark -j

/workspace/gpu-powerflow/build/bench-operators/benchmarks/cupf_case_benchmark \
  --case-dir /workspace/datasets/cuPF_benchmark_dumps/case30_ieee \
  --profile cuda_mixed_edge \
  --warmup 1 \
  --repeats 10
