...
def main() -> None:
    ...
    if args.skip_build:
        end2end_binary = WORKSPACE_ROOT / "build" / "texas-batch-end2end" / "benchmarks" / "cupf_case_benchmark"
        operators_binary = WORKSPACE_ROOT / "build" / "texas-batch-operators" / "benchmarks" / "cupf_case_benchmark"
    else:
        end2end_binary = configure_and_build(WORKSPACE_ROOT / "build" / "texas-batch-end2end", timing=False)
        operators_binary = configure_and_build(WORKSPACE_ROOT / "build" / "texas-batch-operators", timing=True)
