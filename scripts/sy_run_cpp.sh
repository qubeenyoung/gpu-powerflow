#!/usr/bin/env bash
# --------------------------------------------
# Run C++ NewtonPF over .mat case files
# --------------------------------------------
set -euo pipefail

# ====== [설정: 필요시 수정] ======
OUTPUT_DIR="runs/cpp_pf_logs"        # 로그 저장 경로
CPP_BIN="build/core/test_newtonpf"   # C++ 실행 파일 경로
DATASET_DIR="datasets/nr_dataset"    # 소영수정: nr_dataset 디렉터리 (C++용 .npy/.npz 형식)
...
usage() {
  cat <<'USAGE'
사용법:
  ./sy_run_cpp.sh                # DATASET_DIR에서 무작위 케이스 N개 실행
  ./sy_run_cpp.sh -n 10          # 무작위 10개 실행
  ./sy_run_cpp.sh case9 ...      # 인자로 준 케이스들만 실행
  ./sy_run_cpp.sh -l list.txt    # list.txt에 있는 케이스 목록 실행
...
설명:
  • C++ NewtonPF 실행 파일을 사용하여 power flow 계산을 수행합니다.
  • 출력은 케이스별로 ${OUTPUT_DIR}/<케이스이름>/ 디렉터리에 저장됩니다.
  • 먼저 './scripts/sy_build.sh'로 빌드가 필요합니다.
...
# C++ 실행 파일 존재 확인
if [[ ! -f "$CPP_BIN" ]]; then
  echo "에러: C++ 실행 파일을 찾을 수 없습니다: $CPP_BIN"
  echo "먼저 './scripts/build.sh'로 빌드하세요."
  exit 1
fi

# 케이스 목록 만들기 (소영수정: 디렉터리 기반)
...
for CASE_NAME in "${CASE_FILES[@]}"; do
  ...
  # C++ 실행: --case <case_name> --out <output_dir>
  if "$CPP_BIN" --case "$CASE_NAME" --out "$OUTPUT_DIR"; then
    echo "[완료] ${CASE_NAME}"
  else
    echo "[실패] ${CASE_NAME}" >&2
    RET_CODE=1
...
if [[ $RET_CODE -eq 0 ]]; then
  echo "모든 실행이 완료되었습니다. 결과: ${OUTPUT_DIR}"
else
