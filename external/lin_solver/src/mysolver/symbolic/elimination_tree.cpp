                  ...
                  std::vector<int>& Lp, std::vector<int>& Li)
{
    ...
    // 156k small-vector allocs + regrowth -- that churn was the bulk of the cost, cf.
    // cy259's build_symmetric_filled win). column_counts (proven cs_counts) gives the
    ...
    for (int j = 0; j < n; ++j) {
        ...
        for (int t = choff[j]; t < choff[j + 1]; ++t) {
            const int c = chl[t];
            for (int u = Lp[c]; u < Lp[c + 1]; ++u) {  // child structure, read from its slice
                const int i = Li[u];
                if (i != c && mark[i] != j) {
                    mark[i] = j;
