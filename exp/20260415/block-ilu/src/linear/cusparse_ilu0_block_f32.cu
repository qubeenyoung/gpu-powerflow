...
CusparseIlu0BlockF32::CusparseIlu0BlockF32(std::string name)
    : name_(std::move(name))
{
    CUSPARSE_CHECK(cusparseCreate(&handle_));
}

CusparseIlu0BlockF32::~CusparseIlu0BlockF32()
{
    destroy();
}

void CusparseIlu0BlockF32::set_stream(cudaStream_t stream)
{
    stream_ = stream;
    CUSPARSE_CHECK(cusparseSetStream(handle_, stream_));
}

void CusparseIlu0BlockF32::clear_stream_noexcept()
{
    stream_ = nullptr;
    if (handle_ != nullptr) {
...
void CusparseIlu0BlockF32::destroy()
{
    if (vec_out_ != nullptr) {
...
void CusparseIlu0BlockF32::check_analyzed(const char* method) const
{
...
void CusparseIlu0BlockF32::analyze(DeviceCsrMatrixViewF32 matrix)
{
    ...
    CUSPARSE_CHECK(cusparseScsrilu02_bufferSize(handle_,
                                                rows_,
                                                nnz_,
                                                legacy_descr_,
                                                values_,
    ...
    CUSPARSE_CHECK(cusparseScsrilu02_analysis(handle_,
                                              rows_,
                                              nnz_,
                                              legacy_descr_,
                                              values_,
    ...
    CUSPARSE_CHECK(cusparseCreateDnVec(&vec_rhs_, rows_, d_dummy_rhs_.data(), CUDA_R_32F));
    CUSPARSE_CHECK(cusparseCreateDnVec(&vec_tmp_, rows_, d_tmp_.data(), CUDA_R_32F));
    CUSPARSE_CHECK(cusparseCreateDnVec(&vec_out_, rows_, d_dummy_out_.data(), CUDA_R_32F));

    ready_ = true;
}

void CusparseIlu0BlockF32::check_zero_pivot(const char* stage)
{
    last_zero_pivot_ = -1;
...
void CusparseIlu0BlockF32::ensure_spsv_buffer_size(std::size_t required)
{
...
void CusparseIlu0BlockF32::analyze_triangular_solves()
{
...
void CusparseIlu0BlockF32::factorize()
{
    check_analyzed("factorize");

    CUSPARSE_CHECK(cusparseScsrilu02(handle_,
                                     rows_,
                                     nnz_,
                                     legacy_descr_,
                                     values_,
...
void CusparseIlu0BlockF32::solve(const float* rhs_device, float* out_device)
{
    ...
    CUSPARSE_CHECK(cusparseDnVecSetValues(vec_rhs_, const_cast<float*>(rhs_device)));
