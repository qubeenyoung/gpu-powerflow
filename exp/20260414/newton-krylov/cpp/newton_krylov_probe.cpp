...
using exp_20260414::newton_krylov::JfnkLinearSolveBiCGSTAB;
...
void print_usage(const char* argv0)
{
    std::cout
        ...
        << " [--solver bicgstab|fgmres|bicgstab_none|gmres_none]"
        << " [--preconditioner none|bus_block_jacobi_fd|ilut_fd|ilu0_fd|amg_fd]"
...
CliOptions parse_args(int argc, char** argv)
{
    ...
    if (options.jfnk.solver != "bicgstab" &&
        options.jfnk.solver != "fgmres" &&
        options.jfnk.solver != "bicgstab_none" &&
        options.jfnk.solver != "gmres_none") {
        throw std::runtime_error("--solver must be bicgstab, fgmres, bicgstab_none, or gmres_none");
    }
    if (options.jfnk.preconditioner != "none" &&
        options.jfnk.preconditioner != "bus_block_jacobi_fd" &&
        options.jfnk.preconditioner != "ilut_fd" &&
        options.jfnk.preconditioner != "ilu0_fd" &&
        options.jfnk.preconditioner != "amg_fd") {
        throw std::runtime_error(
            "--preconditioner must be none, bus_block_jacobi_fd, ilut_fd, ilu0_fd, or amg_fd");
    }
...
CaseResult run_case(const CliOptions& cli, const std::filesystem::path& case_dir)
{
    ...
    JacobianMaps empty_maps;
    JacobianStructure empty_structure;
    const AnalyzeContext analyze_ctx =
        make_analyze_context(case_data, ybus, empty_maps, empty_structure);

    CpuFp64Storage storage;
    CpuMismatchOpF64 mismatch(storage);
    JfnkLinearSolveBiCGSTAB linear_solve(storage, cli.jfnk);
