...
void validate_case_data(const CupfDatasetCase& data)
{
...
bool is_case_directory(const std::filesystem::path& path)
{
    return std::filesystem::is_directory(path) &&
           std::filesystem::exists(path / "dump_Ybus.mtx") &&
           std::filesystem::exists(path / "dump_Sbus.txt") &&
           std::filesystem::exists(path / "dump_V.txt") &&
           std::filesystem::exists(path / "dump_pv.txt") &&
           std::filesystem::exists(path / "dump_pq.txt");
}
...
{
    if (!is_case_directory(case_dir)) {
        throw std::runtime_error("Invalid cuPF dataset case directory: " + case_dir.string());
    }

    CupfDatasetCase data;
...
std::vector<std::string> list_case_names(const std::filesystem::path& dataset_root)
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator(dataset_root)) {
        if (is_case_directory(entry.path())) {
            case_names.push_back(entry.path().filename().string());
        }
    }
