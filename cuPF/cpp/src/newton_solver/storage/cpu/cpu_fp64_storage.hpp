...
void CpuCscMatrix<T>::setFromTriplets(Iter first, Iter last)
{
    ...
    const int32_t total = indptr[cols_];

    // Pass 2: bucket triplets into per-column slabs.
    std::vector<int32_t> rows_raw(total, 0);
    std::vector<T>       vals_raw(total, T(0));
    std::vector<int32_t> cursor = indptr;
    for (Iter it = first; it != last; ++it) {
        const int32_t pos = cursor[it->col]++;
        rows_raw[pos] = it->row;
        vals_raw[pos] = it->value;
    }
    ...
    std::vector<int32_t> new_indptr(cols_ + 1, 0);
    std::vector<int32_t> perm;

    for (int32_t c = 0; c < cols_; ++c) {
        new_indptr[c] =
            static_cast<int32_t>(indices.size());
        const int32_t lo = indptr[c];
        const int32_t hi = indptr[c + 1];
        ...
        while (i < n) {
            ...
            const int32_t row = rows_raw[pi];
            T             acc = vals_raw[pi];
            int32_t       j   = i + 1;
            while (j < n) {
                const int32_t pj = perm[j];
                if (rows_raw[pj] != row) break;
                acc += vals_raw[pj];
                ++j;
            }
