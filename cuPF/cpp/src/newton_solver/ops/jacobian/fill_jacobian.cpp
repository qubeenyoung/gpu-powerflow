...
void CpuPandapowerJacobianOpF64::run(CpuFp64Storage& buf, IterationContext& ctx)
{
    (void)ctx;

    if (buf.n_bus <= 0 || buf.J.nonZeros() <= 0) {
        throw std::runtime_error("CpuPandapowerJacobianOpF64::run: buffers are not prepared");
    }
    ...
    for (int32_t i = 0; i < buf.n_bus; ++i) {
        const std::size_t idx = static_cast<std::size_t>(i);
        const double vm = std::abs(buf.V[idx]);
        if (vm == 0.0) {
            throw std::runtime_error(
                "CpuPandapowerJacobianOpF64::run: zero voltage magnitude is unsupported by pandapower-like dSbus_dV");
        }
        Vnorm[idx] = buf.V[idx] / vm;
    }
    ...
    for (int32_t row = 0; row < buf.n_bus; ++row) {
        for (int32_t k = buf.Ybus_indptr[static_cast<std::size_t>(row)];
            ...
            const std::complex<double> dS_dVa =
                -kImaginaryUnit * buf.V[static_cast<std::size_t>(y_i)] *
                std::conj(y * buf.V[static_cast<std::size_t>(y_j)]);

            const std::complex<double> dS_dVm =
                buf.V[static_cast<std::size_t>(y_i)] *
                std::conj(y * Vnorm[static_cast<std::size_t>(y_j)]);
            ...
            if (p11 >= 0) J_values[p11] = dS_dVa.real();
            if (p21 >= 0) J_values[p21] = dS_dVa.imag();
            if (p12 >= 0) J_values[p12] = dS_dVm.real();
            if (p22 >= 0) J_values[p22] = dS_dVm.imag();
        }
    }
    ...
    for (int32_t bus = 0; bus < buf.n_bus; ++bus) {
        ...
        if (q11 >= 0) J_values[q11] += dS_dVa.real();
        if (q21 >= 0) J_values[q21] += dS_dVa.imag();
        if (q12 >= 0) J_values[q12] += dS_dVm.real();
        if (q22 >= 0) J_values[q22] += dS_dVm.imag();
    }
}
