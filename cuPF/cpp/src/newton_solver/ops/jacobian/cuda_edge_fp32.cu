// ---------------------------------------------------------------------------
// cuda_edge_fp32.cu — FP32 edge-based Jacobian orchestrator
//
...
void CudaJacobianOpEdgeFp32::run(IterationContext& ctx)
{
    ...
    if (storage.d_Y_row.empty() || storage.d_J_values.empty() || storage.d_Ibus_re.empty()) {
    ...
    launch_fill_jacobian_edge_offdiag_fp32(storage);
