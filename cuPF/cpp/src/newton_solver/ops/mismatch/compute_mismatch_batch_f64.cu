...
//
// Main kernel: compute_mismatch_batch_f64_kernel.
// Packs F64 from V64, Ibus64, and Sbus64 for every batch.
// ---------------------------------------------------------------------------
...
__global__ void compute_mismatch_batch_f64_kernel(
...
void launch_compute_mismatch_batch_f64(CudaMixedStorage& storage)
{
    if (storage.n_bus <= 0 || storage.dimF <= 0 || storage.batch_size <= 0) {
        throw std::runtime_error("launch_compute_mismatch_batch_f64: storage is not prepared");
    }
    ...
    compute_mismatch_batch_f64_kernel<<<grid, block>>>(
        total_entries,
        storage.dimF,
        storage.n_bus,
