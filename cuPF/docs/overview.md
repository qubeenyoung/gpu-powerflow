...
│   │   ├── core/                  # Solver·Context·Plan 인터페이스
│   │   └── ops/                   # Op 추상 인터페이스 및 Jacobian analysis 타입
│   └── src/newton_solver/         # 구현
│       ├── core/                  # NewtonSolver, solver stage configuration
...
`NewtonSolver` 생성자가 `solver stage configuration::build(options)`를 호출해 `NewtonSolver stage ownership`을 조립한다.
이후 `initialize()`·`solve()` hot path에서는 인터페이스 가상 함수만 호출하며, backend/precision 분기가 없다.
...
```
NewtonSolver solver(options);   // NewtonSolver stage ownership 조립
...
| CUDA FP64      | CUDA    | FP64    | Edge one-pass    | cuDSS FP64      |
| CUDA Mixed     | CUDA    | Mixed   | Edge one-pass    | cuDSS FP32      |

Mixed 프로파일은 고정 구성이다. `Ybus`, `Va/Vm` 전압 상태, `V_re/V_im` cache, `Sbus`,
`Ibus`, `F`는 FP64로 유지하고, `J/dx`는 FP32로 둔다.
...
| [core/README.md](core/README.md) | NewtonSolver, Jacobian analysis, NewtonSolver stage ownership, Context |
