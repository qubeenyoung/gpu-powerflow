...
`JacobianBuilder::analyze()`는 이 패턴을 한 번 분석하고, Ybus 원소 k번째가 J의 어느 CSR 위치에 기여하는지를 `mapJ11/12/21/22`에 기록한다. 이후 NR 반복마다 이 맵으로 직접 scatter한다.
...
| Stage          | 정밀도 | 이유 |
|---------------|--------|------|
| Mismatch      | FP32 Ybus + FP64 V/Ibus/Sbus/F | Ybus bandwidth를 줄이되 전압, Ibus 저장, 지정 전력, normF는 FP64 |
| Jacobian fill | FP32 내부 계산 | FP64 `V/Ibus` 입력을 커널 안에서 FP32로 cast하고 `J_values`는 FP32로 기록 |
| Linear solve  | FP32   | cuDSS FP32로 충분히 수렴 가능 |
| Voltage update| FP64 Va/Vm / FP64 V cache | 상태 벡터(θ, \|V\|)와 rectangular cache를 FP64로 유지 |

CUDA convention에서는 `J * dx = F`를 풀고 FP32 dx를 FP64 Va/Vm에서 뺀다.
전압 cache `V_re/V_im`도 FP64로 재구성하고, 최종 public output은 FP64 Va/Vm에서 만든다.
