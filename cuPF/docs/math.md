...
| Stage          | 정밀도 | 이유 |
|---------------|--------|------|
| Mismatch      | FP64   | 수렴 판정 기준(normF)이 정확해야 함 |
| Jacobian fill | FP32   | 행렬 원소 계산은 FP32 오차가 허용됨 |
| Linear solve  | FP32   | cuDSS FP32로 충분히 수렴 가능 |
| Voltage update| FP64   | 상태 벡터(θ, \|V\|) 누산 오차 방지 |

FP32 dx를 FP64 Va/Vm에 더하는 과정에서 암묵적 형변환이 일어나므로 전압 상태는 FP64 정밀도를 유지한다.
