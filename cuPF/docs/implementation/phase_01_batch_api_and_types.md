...
3. `solver stage configuration`는 여전히 backend/compute 조합만 선택한다.
...
- [cpp/src/newton_solver/core/solver_contexts.hpp](../../cpp/src/newton_solver/core/solver_contexts.hpp)
...
single-case `NRResult`는 `B=1` 결과를 unwrap한 wrapper다.

### 3. analyze API는 구조 중심 유지

`InitializeContext`는 계속 sparse structure와 maps 중심으로 유지한다.
값 dtype/layout 문제는 `SolveContext`와 storage upload 단계에서 처리한다.

### 4. profile 선택은 그대로 유지

`solver stage configuration`는 아래 축만 계속 본다.
...
- `BatchNewtonSolver stage ownership`을 만들지 않는다.
...
- `solver stage configuration`의 의미가 바뀌지 않는다.
