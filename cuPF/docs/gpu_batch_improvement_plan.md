...
이 가정에서는 `initialize()` 결과를 배치 전체가 공유할 수 있다. 즉 Jacobian structure, scatter maps, cuDSS symbolic analysis 입력 구조는 한 번 만든 뒤 재사용한다.
...
전압 상태는 `Va/Vm`을 FP64 authoritative state로 둔다. 현재 경로는
`Ybus64 * V64 -> Ibus64` 정책으로 mismatch용 `V_re/V_im` derived cache도 FP64로 둔다.
Jacobian은 FP64 `Ybus`, FP64 전압 cache와 FP64 `Ibus`를 읽어 커널 안에서 FP32로 변환한다.
초기에는 `V_re/V_im` FP32 cache가 Texas Univ `case_ACTIVSg200` B=1 수렴성을 악화시켰고,
이후 FP64 cache로 개선됐다. 현재는 mismatch/public 경계와 Jacobian 입력 cache를 분리해
수렴성은 FP64 cache로 지키고 Jacobian 값 계산은 FP32로 유지한다.

```
authoritative state: Va, Vm       // double
derived cache:       V_re, V_im   // double
reusable current:     Ibus_re/im   // double
```

Voltage update는 `Va/Vm`을 갱신하고, 갱신된 FP64 `V_re/V_im` cache와
FP64 `Vm` cache를 유지한다. 이후 mismatch는 `V_re/V_im`을 읽고,
Jacobian은 `V_re/V_im`, `Vm`과 mismatch가 만든 FP64 `Ibus`를 읽는다.

장점:
- update 단계에서 매번 `V_re/V_im -> Va/Vm` 분해 커널을 실행할 필요가 없다.
- mismatch/Jacobian은 복소 전압 cache를 바로 읽을 수 있어 `sincos(Va)` 반복 계산을 피한다.
- slack bus 상태는 갱신되지 않지만 현재 구현은 batch 전체 `V_re/V_im` cache를 재구성한다.
- 기존 CUDA 커널의 입력 형태와 잘 맞아 이행 비용이 낮다.

단점:
- 전압 상태가 `Va/Vm`과 `V_re/V_im` FP64 배열을 함께 들고 있어 메모리를 더 쓴다.
...
최종 `NRResult.V`를 만들 때는 FP64 `Va/Vm`에서 재구성해 public output 정밀도를 유지한다.
...
권장 구조는 `Va/Vm`을 authoritative state로 유지하고, update 후 `V_re/V_im` cache를 재구성하는 것이다.
...
Ibus64 = custom_batch_csr_spmv(Ybus64, V64)
...
```
kernel A: Ibus64 = custom_batch_csr_spmv(Ybus64, V64)
kernel B: F64 = V64 * conj(Ibus64) - Sbus64, norm reduction input 작성
kernel C: Jacobian edge one-pass fill
```
...
현재 mixed edge path는 diagonal Ybus entry thread가 self term을 direct write하고,
`Ibus` 기반 correction을 같은 kernel에서 더하는 one-pass 구조다. 이 구조에서는 edge diagonal
atomic과 별도 diag launch를 모두 피할 수 있다.

`d_J_values.memsetZero()`를 제거하려면 모든 J entry가 매 iteration 정확히 한 번 write되는지
poison/coverage 검증이 필요하다.
...
- 별도 `CudaBatch*` class, `BatchNewtonSolver stage ownership`, `Batchsolver stage configuration`는 만들지 않는다. 기존 CUDA plan이 batch dimension을 갖고 `B=1`도 같은 경로로 실행한다.
...
1. 완료: `F = S_calc - S_spec`, `J * dx = F`, `state -= dx` 공통 convention 정리
...
6. 변경: Mixed `Ybus`, `Sbus`, `Ibus`, voltage cache는 FP64 유지
7. 완료: Ibus custom batch CSR kernel, mixed edge one-pass Jacobian
8. 진행 전: edge `d_J_values.memsetZero()` 제거
...
edge one-pass Jacobian numerical diff
FP64 Ybus + FP64 V/Ibus, FP32 J/dx profile의 실패 케이스 목록
