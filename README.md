...
이 저장소는 GPU 기반 전력 조류 계산 실험을 위한 작업 공간이다. 현재 기준으로 핵심 코드는 [`cuPF`](/workspace/cuPF) 아래에 있고, 루트에는 빌드/실행 보조 스크립트와 실험 자산이 함께 정리되어 있다.

## 현재 버전 요약

- 주 구현 대상은 `cuPF` C++/CUDA 전력 조류 솔버 스냅샷이다.
...
[`cuPF`](/workspace/cuPF)는 희소 `Ybus`를 입력으로 받아 Newton-Raphson 방식으로 전력 조류를 계산하는 C++/CUDA 라이브러리다.
...
더 자세한 구현 설명과 제약 사항은 [`cuPF/README.md`](/workspace/cuPF/README.md)를 보면 된다.
...
- [`build_cupf.bash`](/workspace/bash/build_cupf.bash): CMake preset 기반 빌드
- [`run_cupf.bash`](/workspace/bash/run_cupf.bash): smoke 실행 래퍼
...
정식 빌드 옵션과 의존성은 [`cuPF/README.md`](/workspace/cuPF/README.md)에 정리되어 있다.

## 주의 사항

- 이 저장소에는 실험 코드와 로컬 분석 산출물이 함께 있다.
- 리팩터링 중인 영역은 문서, 바인딩, 벤치마크 하니스가 완전히 동기화되지 않았을 수 있다.
- 재현 가능한 기준은 우선 `cuPF`의 single-case CPU/CUDA smoke 경로로 보는 것이 안전하다.
