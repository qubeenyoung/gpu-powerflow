...
def _cpp_command(args: argparse.Namespace, variant: dict[str, Any], executable: Path, dump_root: Path, raw_out: Path) -> list[str]:
    cmd = [
        ...
        "--custom-precision",
        variant.get("custom_precision", "fp32"),
        "--custom-serial-nd",
        "1" if variant.get("custom_serial_nd", False) else "0",
        "--custom-seed",
        str(variant.get("custom_seed", 42)),
        "--tolerance",
        str(args.tolerance),
        "--max-iter",
        str(args.max_iter),
...
def run_variant(args: argparse.Namespace, variant: dict[str, Any]) -> None:
    ...
    manifest = eval_common.manifest(
        ...
        compute=variant["compute"],
        linear_solver=variant["linear_solver"],
        custom_precision=variant.get("custom_precision", ""),
        custom_serial_nd=variant.get("custom_serial_nd", ""),
        custom_seed=variant.get("custom_seed", ""),
        custom_ozaki=variant.get("custom_ozaki", ""),
        jacobian=variant.get("jacobian", ""),
        entrypoint="native",
