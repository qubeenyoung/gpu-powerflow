...
struct Options {
  ...
  int repeat = 1;
  int warmup = 0;  // untimed factor/Solve iterations discarded before the timed
                   // `repeat` set
  int batch =
...
class DeviceBuffer {
 public:
  ...
  void upload(const std::vector<T>& host) {
    ...
    cuda_check(
        cudaMalloc(reinterpret_cast<void**>(&ptr_), host.size() * sizeof(T)),
        "cudaMalloc");
...
void usage(const char* argv0) {
  std::cerr
      ...
      << "  --warmup N            untimed factor/Solve iters discarded before "
         "timing\n"
      << "  --single-precision fp64|fp32\n"
      << "                        single-system (non-batch) input precision\n"
      << "  --batch B             also run a uniform-batch experiment with B "
         "systems\n"
      ...
      << "                          tf32       (TF32 Ozaki mma trailing, "
         "recommended)\n"
      << "  --max-panel-width N         Analyze: max panel width inside a "
         "supernode (1..64)\n"
      << "  --serial-nd          use deterministic serial METIS NodeND\n"
      << "  --Analyze-only        stop after Analyze (useful with "
         "--dump-fronts)\n"
...
Options parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--matrix") {
      ...
      if (++i >= argc)
        throw std::runtime_error("--solution-out requires a path");
      ...
      if (++i >= argc)
        throw std::runtime_error("--single-precision requires fp64 or fp32");
      const std::string value = argv[i];
      if (value == "fp64")
        options.single_fp32 = false;
      else if (value == "fp32")
        options.single_fp32 = true;
      else
        throw std::runtime_error("--single-precision must be fp64 or fp32");
    } else if (arg == "--max-panel-width") {
      if (++i >= argc)
        throw std::runtime_error("--max-panel-width requires an int");
      ...
      if (++i >= argc)
        throw std::runtime_error("--dump-fronts requires a path");
      options.dump_fronts_path = argv[i];
    } else if (arg == "--precision") {
      if (++i >= argc)
        throw std::runtime_error("--precision requires fp64|fp32|tf32");
      const std::string value = argv[i];
      using cls::Precision;
      if (value == "fp64")
        ...
        options.precision = Precision::TF32;
      else
        throw std::runtime_error("--precision must be fp64|fp32|tf32");
  ...
  if (!case_dir.empty()) {
    if (!options.matrix_path.empty() || !options.rhs_path.empty()) {
      throw std::runtime_error(
          "use either <case-dir> or --matrix/--rhs, not both");
    }
...
void require_success(cls::Status status, const char* phase) {
  if (status != cls::Status::kSuccess) {
    throw std::runtime_error(std::string(phase) +
                             " failed: " + cls::StatusString(status));
  }
}
...
ResidualStats residual(const scripts::CsrMatrix& matrix,
...
int main(int argc, char** argv) {
  try {
    ...
    scripts::CsrMatrix matrix =
        scripts::read_matrix_market_csr(options.matrix_path);
    scripts::DenseVector rhs =
        scripts::read_matrix_market_vector(options.rhs_path);
    ...
    solver_config.use_parallel_nested_dissection =
        options.use_parallel_nested_dissection;
    if (options.max_panel_width > 0)
      solver_config.max_panel_width = options.max_panel_width;
    ...
    if (!options.dump_fronts_path.empty()) {
      solver_config.analyze_dump_fronts_path =
          options.dump_fronts_path.string();
    }
    ...
    matrix_view.value_type = options.single_fp32 ? cls::ValueType::kFloat32
                                                 : cls::ValueType::kFloat64;
    ...
    rhs_view.value_type = options.single_fp32 ? cls::ValueType::kFloat32
                                              : cls::ValueType::kFloat64;
    rhs_view.values = options.single_fp32 ? static_cast<void*>(d_rhs_f.get())
    ...
    solution_view.value_type = options.single_fp32 ? cls::ValueType::kFloat32
                                                   : cls::ValueType::kFloat64;
    solution_view.values = options.single_fp32
                               ? static_cast<void*>(d_solution_f.get())
                               : static_cast<void*>(d_solution.get());

    require_success(solver.SetData(matrix_view), "SetData");
    require_success(solver.SetRhs(rhs_view), "SetRhs");
    require_success(solver.SetSolution(solution_view), "SetSolution");
    ...
    if (options.analyze_only) {
      std::cout
          ...
          << '\n'
          << "read_ms="
          << std::chrono::duration<double, std::milli>(t_read - t0).count()
          << '\n'
          << "upload_ms="
          << std::chrono::duration<double, std::milli>(t_upload - t_read)
                 .count()
          << '\n'
          << "set_ms="
          << std::chrono::duration<double, std::milli>(t_set - t_upload).count()
          << '\n'
          << "analyze_ms="
          << std::chrono::duration<double, std::milli>(t_analyze - t_set)
                 .count()
          << '\n';
      return 0;
    }
    ...
    if (options.batch_only) {
      factor_ms.push_back(0);
      solve_ms.push_back(0);
    }
    ...
    for (int r = 0; r < options.repeat && !options.batch_only; ++r) {
      ...
      cuda_check(cudaDeviceSynchronize(),
                 "cudaDeviceSynchronize after Factorize");
    ...
    if (options.batch > 0) {
      ...
      // B identical copies (same pattern + values) -> each batch solves the
      // same system, so the per-batch residual must match the single-system
      // Solve (correctness check).
      std::vector<double> hvalB((std::size_t)B * nnz),
          hrhsB((std::size_t)B * n);
      for (int b = 0; b < B; ++b) {
        std::copy(matrix.values.begin(), matrix.values.end(),
                  hvalB.begin() + (std::size_t)b * nnz);
        std::copy(rhs.values.begin(), rhs.values.end(),
                  hrhsB.begin() + (std::size_t)b * n);
      }
      ...
      // Register the batched buffers (size = B * nnz / B * n) and run the phase
      // API.
      ...
      require_success(solver.SetData(batched_matrix), "SetData(batched)");
      require_success(solver.SetRhs(batched_rhs), "SetRhs(batched)");
      require_success(solver.SetSolution(batched_sol), "SetSolution(batched)");
      ...
      std::cout << "setup_ms=" << setup_ms
                << " (one-time, outside Newton loop)\n";

      // Warmup (untimed): same purpose as the single-system path — drop
      // graph-instantiation and lazy-alloc cost from the first measured
      // iteration.
      ...
      for (int r = 0; r < options.repeat; ++r) {
        ...
        bf.push_back(
            std::chrono::duration<double, std::milli>(ef - sf).count());
        bs.push_back(
            std::chrono::duration<double, std::milli>(es - ef).count());
      }
      batch_factor_per_ms = median(bf) / B;
      batch_solve_per_ms = median(bs) / B;
      std::vector<double> solB((std::size_t)B * n);
      cuda_check(cudaMemcpy(solB.data(), d_solB.get(),
                            (std::size_t)B * n * sizeof(double),
                            cudaMemcpyDeviceToHost),
                 "D2H batch sol");
      // max relres over ALL B batches (catches per-batch indexing bugs, not
      // just batch 0)
      for (int b = 0; b < B; ++b) {
        std::vector<double> xb(solB.begin() + (std::size_t)b * n,
                               solB.begin() + (std::size_t)(b + 1) * n);
        batch_relres =
            std::max(batch_relres, residual(matrix, rhs.values, xb).rel_l2);
      }
    }
    ...
    std::cout << "download_check_ms=" << ms(t_solve, t_done) << '\n'
