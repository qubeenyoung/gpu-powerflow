...
int main(int argc, char** argv)
{
    try {
        ...
        DeviceBuffer<float> d_rhs;
        DeviceBuffer<float> d_solution;
        ...
        std::vector<float> rhs_f(rhs.values.begin(), rhs.values.end());
        {
            ...
            d_values.upload(values_f);
            d_rhs.upload(rhs_f);
            d_solution.allocate(rhs.values.size());
            cuda_check(cudaMemset(d_solution.get(), 0, rhs.values.size() * sizeof(float)),
                       "cudaMemset solution");
        }
        ...
        CudssState state;
        {
            ...
            cudss_check(cudssMatrixCreateCsr(
                            ...
                            nullptr, d_col_idx.get(), d_values.get(), CUDA_R_32I, CUDA_R_32F,
            ...
            cudss_check(cudssMatrixCreateDn(&state.solution, matrix.cols, 1, matrix.cols,
                                            d_solution.get(), CUDA_R_32F, CUDSS_LAYOUT_COL_MAJOR),
        ...
        ResidualStats stats;
        {
            ...
            std::vector<float> sol_f = d_solution.download();
            solution.assign(sol_f.begin(), sol_f.end());
