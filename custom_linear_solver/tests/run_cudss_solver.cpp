...
void usage(const char* argv0) {
  std::cerr
      ...
      << "  --batch B               uniform-batch B systems "
         "(CUDSS_CONFIG_UBATCH_SIZE);\n"
      << "                          shares one sparsity pattern; default 1\n"
      << "  --repeat N              time-averaging trials (median reported)\n"
      << "  --mt-auto               cudssSetThreadingLayer(handle, nullptr) — "
         "let cuDSS\n"
      << "                          load its default threading layer (cuPF's "
         "pattern)\n"
      << "  --mt                    same as --mt-auto but uses --threading-lib "
         "path\n"
      ...
      << "  --split-analysis        report reordering vs symbolic "
...
std::string default_threading_layer() {
  ...
  // Search for libcudss_mtlayer_*.so in conventional locations. The /opt path
  // is the system-wide install; the python-package path is the pip-installed
  // copy.
  const fs::path candidates[] = {
      "/opt/nvidia/cudss/lib/libcudss_mtlayer_gomp.so",
      "/opt/nvidia/cudss/lib/libcudss_mtlayer_gomp.so.0",
      "/usr/local/lib/python3.10/dist-packages/nvidia/cu12/lib/"
...
Options parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--matrix") {
      ...
      if (++i >= argc)
        throw std::runtime_error("--threading-lib requires a path");
      options.threading_lib = argv[i];
      options.use_threading_layer = true;
    } else if (arg == "--host-nthreads") {
      if (++i >= argc)
        throw std::runtime_error("--host-nthreads requires a count");
      ...
      if (++i >= argc)
        throw std::runtime_error("--precision requires fp32 or fp64");
...
int main(int argc, char** argv) {
  try {
    ...
    void* p_solution = nullptr;
    {
      ...
      // Replicate values / RHS B times so cuDSS sees B systems with the same
      // sparsity.
      if (options.fp32) {
        ...
        cuda_check(cudaMemset(d_solution_f.get(), 0,
                              (std::size_t)B * n * sizeof(float)),
                   "cudaMemset solution");
        ...
        cuda_check(cudaMemset(d_solution.get(), 0,
    ...
    CudssState state;
    {
      ...
      if (options.use_threading_layer) {
        // --mt-auto matches cuPF's intent (let cuDSS pick the default mt
        // layer). The current cuDSS build rejects nullptr, so we resolve the
        // bundled libcudss_mtlayer_*.so path the same way
        // default_threading_layer() does.
        if (options.mt_auto) {
          threading_lib = default_threading_layer();
        } else {
          threading_lib = options.threading_lib.empty()
                              ? default_threading_layer()
      ...
      cudss_check(cudssConfigSet(state.config, CUDSS_CONFIG_REORDERING_ALG,
      ...
      if (options.use_matching) {
        int use_matching = 1;
        cudss_check(cudssConfigSet(state.config, CUDSS_CONFIG_USE_MATCHING,
      ...
      cudss_check(cudssMatrixCreateCsr(&state.matrix, matrix.rows, matrix.cols,
                                       matrix.nnz(), d_row_ptr.get(), nullptr,
                                       d_col_idx.get(), p_values, CUDA_R_32I,
                                       cuda_dtype, CUDSS_MTYPE_GENERAL,
                                       CUDSS_MVIEW_FULL, CUDSS_BASE_ZERO),
                  "cudssMatrixCreateCsr");
      cudss_check(
          cudssMatrixCreateDn(&state.rhs, matrix.rows, 1, matrix.rows, p_rhs,
                              cuda_dtype, CUDSS_LAYOUT_COL_MAJOR),
          "cudssMatrixCreateDn rhs");
      cudss_check(
          cudssMatrixCreateDn(&state.solution, matrix.cols, 1, matrix.cols,
    ...
    if (options.split_analysis) {
      {
        ...
        cudss_check(
            cudssExecute(state.handle, CUDSS_PHASE_REORDERING, state.config,
      ...
      }
      {
        const auto start = std::chrono::steady_clock::now();
        cudss_check(
            cudssExecute(state.handle, CUDSS_PHASE_SYMBOLIC_FACTORIZATION,
      ...
      cudss_check(
          cudssExecute(state.handle, CUDSS_PHASE_ANALYSIS, state.config,
                       state.data, state.matrix, state.solution, state.rhs),
          "cuDSS analysis");
    ...
    for (int r = 0; r < options.repeat; ++r) {
      ...
      const auto start = std::chrono::steady_clock::now();
      cudss_check(
          cudssExecute(state.handle, CUDSS_PHASE_FACTORIZATION, state.config,
                       state.data, state.matrix, state.solution, state.rhs),
          "cuDSS factorization");
      cuda_check(cudaDeviceSynchronize(),
    ...
    for (int r = 0; r < options.repeat; ++r) {
      ...
      const auto start = std::chrono::steady_clock::now();
      cudss_check(
          cudssExecute(state.handle, CUDSS_PHASE_SOLVE, state.config,
                       state.data, state.matrix, state.solution, state.rhs),
          "cuDSS solve");
    ...
    double batch_max_relres = 0.0;
    {
      if (options.fp32) {
        ...
        // Max relres over all B systems (sanity check; all B copies are
        // identical so this should match the batch-0 number except for rounding
        // noise).
        std::vector<double> xb(n);
        for (int b = 0; b < B; ++b) {
          std::transform(sf.begin() + (std::size_t)b * n,
                         sf.begin() + (std::size_t)(b + 1) * n, xb.begin(),
