...
// FACTORIZE — LARGE tier (front too large for shared; stays global-resident). Stages only the L/U
                                                              ...
                                                              int abase = 0)
{
    ...
    // Fall-through path for uc_pad > 64 (large-tier front strips wider than NTJ8_MAX × 8). The
...
// Unified LARGE kernel for every precision (front too large for the shared-resident small/big
...
__global__ void factor_large(int lbegin, int lend, const int* __restrict__ plcols,
                                   ...
                                   double pivot_shift)
{
    ...
    extern __shared__ char smem_large_unified[];
    bool fused;
    if constexpr (UseTC) {
...
static void dispatch_factor_large(const MultifrontalPlan& plan, State& st, cudaStream_t stream,
