...
// FACTORIZE — MID tier (front fits the per-precision opt-in shared budget at full occupancy). The
...
#include "factorize/mid_ablation.cuh"   // CLS_MID_ABLATION=1 → MAGMA/STRUMPACK-style ablation
...
__global__ void factor_mid(int lbegin, int lend,
                                   ...
                                   double pivot_shift)
{
    ...
    // Phase 3 trailing, with the fsz<=kFusedMidFrontMax fused Phase 1+3 fast path).
    bool did_tc = false;
    if constexpr (UseTC) {
    ...
    if (!did_tc) {
        if (fsz <= kFusedMidFrontMax) {
            lu_mid_front<T>(Fs, fsz, nc, t, nt, sing, static_pivoting,
...
// MID tier — whole-front shared-resident kernel (factor_mid). The entire fsz×fsz front is
// staged into shared and factorized in place; TF32 runs the blocked Tensor-Core trailing. The tier
// boundary (fsz <= kMidFrontMax = 64) guarantees the whole front fits the shared budget for every
...
static void dispatch_factor_mid(const MultifrontalPlan& plan, State& st, cudaStream_t stream,
                                  ...
                                  const FrontRangeCaps& caps)
{
    ...
    // operation-separated, global-resident path (4 launches). Same result; the gap is factor_mid's
    // contribution. (env CLS_MID_ABLATION=1)
    if (mid_ablation_enabled()) {
    ...
    if (precision == Precision::FP64) {
        factor_mid<double, false><<<grid, threads, shared_bytes, stream>>>(
        ...
        factor_mid<float, true><<<grid, threads, shared_bytes, stream>>>(
            b, e, d_plc, plan.d_front_off, plan.d_front_ptr, plan.d_ncols,
            plan.d_panel_parent, plan.d_asm_ptr, plan.d_asm_local, st.d_front_batch_f,
            plan.front_total, st.d_sing, do_extend, fsz_cap, rb_width,
            st.static_pivoting, st.pivot_threshold, st.pivot_shift);
    } else {  // FP32
        factor_mid<float, false><<<grid, threads, shared_bytes, stream>>>(
            b, e, d_plc, plan.d_front_off, plan.d_front_ptr, plan.d_ncols,
            plan.d_panel_parent, plan.d_asm_ptr, plan.d_asm_local, st.d_front_batch_f,
            plan.front_total, st.d_sing, do_extend, fsz_cap, rb_width,
            st.static_pivoting, st.pivot_threshold, st.pivot_shift);
