                                   ...
                                   int do_extend, int fsz_cap)
{
    ...
    if (!did_tc) {
        if (fsz <= 48) {
            ...
            u_panel_solve<T>(Fs, fsz, nc, uc, t, nt);                // Phase 2
            trailing_update_scalar<T>(Fs, fsz, nc, uc, t, nt);       // Phase 3
        }
    }
                                ...
                                const FrontRangeCaps& caps)
{
    ...
    const long blocks = (long)level_size * B, sms = factor_num_sms();
    ...
    if (precision == Precision::FP64) {
        factor_mid_blocked<double, false><<<grid, threads, shared_bytes, stream>>>(
            ...
            plan.front_total, st.d_sing, do_extend, fsz_cap);
