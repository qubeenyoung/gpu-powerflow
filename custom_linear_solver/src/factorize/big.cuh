...
// the thin-K mma trailing. Kernel + staged/TC trailing + dispatch, all here.
...
__device__ __forceinline__ void TrailingUpdateStaged(
    ...
    int abase = 0) {
  // (a) Copy the L (uc × nc) and U (nc × uc) panels of F into compact shared
  // layouts. The
  //     staged layout has unit-strided inner dimensions so the inner
  //     dot-product loop hits contiguous shared lines.
  ...
  __syncthreads();
  // (b)/(c) Each thread owns one (i, j) output, reads from the staged panels,
  // and subtracts.
...
// Bounded-shared trailing for the few oversized separator fronts whose full
// 2·nc·uc staging would exceed the opt-in shared budget (large uc, e.g. circuit
...
// panel straight from global (cheap, coalesced in k). The common power-grid
// path keeps TrailingUpdateStaged unchanged — this is selected only when
// full_sh > budget.
template <typename T, bool FuseExtend = false>
__device__ __forceinline__ void TrailingUpdateStagedTiled(
  ...
  for (int j0 = 0; j0 < uc; j0 += jt) {
    ...
    __syncthreads();  // protect sh_U from the previous tile's readers before
                      // restaging
...
//
// Per-lane register-direct version of the TF32 trailing. The 16×8 output of one
// mma.m16n8k8 instruction places four FP32 accumulator elements in registers
// c0..c3 of each lane, at fixed (row, col) offsets relative to the warp's tile
// origin. Knowing those offsets at compile time lets each lane subtract its
// four entries straight from F under a single bounds check, eliminating the
// wmma::store_matrix_sync → Csc → reload round-trip used by the WMMA variants.
//
// One warp owns one 16-row M-strip (`ti`) of the trailing block, sweeps all N=8
// column tiles (`tj8`), and accumulates across the K dimension (`kc`). The loop
// ordering is (ti, kc, tj8) so the A-fragments — which only depend on (ti, kc)
// — are loaded once per K-tile and reused across the inner tj8 sweep. The
// unrolled tj8 loop keeps the accumulators `c[tj8][...]` in named registers,
// which is required for the CUDA inline-asm
// "+f" operand binding to be stable.
//
// Per-lane A-matrix register layout (verified by probe; inner stride is
// M-block, NOT K):
...
//
// The explicit wmma::__float_to_tf32 conversion is skipped: mma's `.tf32` ABI
// truncates the low 13 bits of the FP32 input automatically, so the explicit
// round-to-nearest only changes the sign of the rounding error within TF32
// precision (within the accuracy budget for the power-grid Solve targets).
...
__device__ __forceinline__ void TrailingUpdateTf32Tc(
  ...
  const int utf_stride =
      uc_pad + 4;  // Utf column stride padded by 4: 2·uc_pad is a multiple of
                   // 32, which made the B-read a 4-way shared bank conflict; +4
                   // spreads laneC across banks 0/8/16/24 → conflict-free.

  // (a) Stage L → Ltf and U → Utf, padded with zeros, no explicit TF32
  // conversion.
  ...
  // `c[<const>][.]` binds to dedicated registers; otherwise nvcc would spill
  // the accumulator to local memory and the "+f" operand binding would
  // misbehave.
...
#pragma unroll
  ...
  // Fall-through path for uc_pad > 64 (big-tier front strips wider than
  // NTJ8_MAX × 8). The (ti, tj8, kc) ordering reloads A on every tj8, but the
  // absolute A-reuse savings are smaller relative to the per-tile work at this
  // size, so the simpler form wins.
  for (int ti = warp; ti < m_tiles; ti += nwarp) {
    const int r_top = ti * 16 + laneR;
    const int r_bot = r_top + 8;
...
// shared-resident small/big kernels; stays global-resident). Phase-3 trailing
// stages the L/U panels into shared once and drains the contribution block
// straight into the parent front (fused trail+extend, baked in — removes the
// uncoalesced CB global write+read round-trip; docs note 30/53). UseTC (T =
// float, TF32) runs the TF32 mma trailing on eligible shapes (nc≤32, uc≤256)
// and scalar otherwise; FP64/FP32 run the staged-scalar trailing. Large fronts
// are always fsz>kFusedMidFrontMax (no LuMidFront fast path), so the kernel
// runs panel LU -> U-Solve -> trailing in order and the fused drain is
// unconditional. Supersedes factor_big_staged<T> and factor_big_tf32_ptx. The
// L/U staging (2·nc·uc·sizeof(T)) always fits the 96 KB opt-in budget on
// power-grid Jacobians, so there is no non-staged fallback.
...
__global__ void FactorBig(
  ...
  if constexpr (UseTC) {
    ...
    (void)level_max_nc;
    (void)level_max_uc;
    // Big fronts are always fsz>kFusedMidFrontMax, so no fused Phase 1+3 fast
    // path: panel LU, U-Solve, trailing.
    LuPanelFactor<float>(F, fsz, nc, t, nt, sing, static_pivoting,
                         pivot_threshold, pivot_shift);  // Phase 1
    UPanelSolveFewsync<float>(F, fsz, nc, uc, t,
                              nt);  // Phase 2 (barrier-cut)
    if (!tc)
    ...
    LuPanelFactor<T>(F, fsz, nc, t, nt, sing, static_pivoting, pivot_threshold,
                     pivot_shift);                 // Phase 1
    UPanelSolveFewsync<T>(F, fsz, nc, uc, t, nt);  // Phase 2 (barrier-cut)
    if (trail_jt > 0) {
...
//
//  The single-block FactorBig is under-fill bound on big-front matrices: a
//  level can hold only a few large separator fronts (ncu: grid=2 on
//  parabolic_fem), so one block per front leaves the GPU >99% idle while each
//  block grinds a uc²·nc trailing serially. Split the big-tier FP64 work into
//  two launches so the embarrassingly-parallel trailing fans out across the
//  whole GPU:
//    (1) FactorBigPanel : one block per front, does Phase-1 panel LU +
//    Phase-2 U-Solve only. (2) FactorBigTrail : a 3D grid (tile, front,
//    batch) — each block owns one TM×TN tile of the
//        front's uc×uc trailing, reads L/U from global (written by kernel 1,
//        ordered by the graph edge between the two launches), and fuses the
//        contribution straight into the parent.
//  Power-grid fronts never reach this tier, so this path is exercised only by
//  circuit/2D-FEM-like matrices with large separators.
// ---------------------------------------------------------------------------------------
template <typename T>
...
// which became the bottleneck once the trailing was multi-blocked: it did ~1/20
// of the trailing's FLOPs but took more time — under-fill + serial nc-loop, ncu
// grid 18-32 / occ 17% / SM 5-12%). The L21/U12 panel solves are parallel over
// the uc rows/cols, so split into:
//   FactorBigPivot  : factor the nc×nc A11 block only (one block/front,
//   small, serial nc). FactorBigPanels : multi-block over uc — L21 =
//   A21·U11⁻¹ (per row) and U12 = L11⁻¹·A12 (per
//                         col), forward-substitution against the staged pivot
//                         block. Fills the GPU.
template <typename T>
__global__ void FactorBigPivot(
  ...
  T* F = front + front_off[p];
  const int t = threadIdx.x, nt = blockDim.x;
  // No-pivot (or static-shift) LU of the nc×nc top-left block only (rows/cols <
  // nc).
  for (int k = 0; k < nc; ++k) {
...
__global__ void FactorBigPanels(int lbegin, int lend,
  ...
  __syncthreads();
  if (g >= uc) return;
  // L21 row g: forward-subst against U11 (cols), in place. L[k] =
  ...
  // U12 col g: forward-subst against L11 (unit lower), in place. U[k] =
...
// BIG tier — the single global-resident kernel for every precision (fronts too
// large to be shared-resident at all: fsz > WholeFrontSharedMax). FactorBig
// keeps the front in global memory, stages only the L/U panels into shared,
// runs the trailing (TF32 mma for UseTC / staged scalar for FP64/FP32), and
// drains the contribution block straight into the parent (fused, baked in). The
// L/U staging always fits the opt-in budget on power-grid Jacobians, so there
// is no non-staged fallback. Routed deterministically by tier — no
// occupancy/opt-in gate.
static void DispatchFactorBig(const MultifrontalPlan& plan, State& st,
  ...
  // Blocked-LU big tier (all precisions): pivot(nc×nc, per-front) → L21/U12
  // panels(multi-block) → trailing(multi-block). Graph edges order the three
  // launches. Only the small pivot block is per-front; everything else fills
  // the GPU. (uc index space uses level_max_uc, unclamped.)
  ...
  if (precision == Precision::FP64) {
    ...
    // grid.x must cover the BIGST front's uc² (level_max_uc is unclamped;
    // max_uc is clamped to the TF32 tile cap and would silently drop tiles
    // beyond it → wrong result).
  ...
  // FP32 and TF32: float fronts → the same multi-block scalar-float big-tier
  // path. The TF32 TC trailing lives in the big tier (fsz≤111, its real
  // payoff); for genuinely large fronts the multi-block scalar path fills the
  // GPU at full FP32 accuracy. This also fixes the prior shared-budget faults
  // on big fronts (FP32 fit the full 2·nc·uc staging only by the luck of 4-byte
  // elements; TF32 staged the per-front uc into max_uc-clamped shared and
  // crashed).
  (void)max_uc;
