...
// FACTORIZE — SMALL tier (max_fsz <= kSmallFrontMax = warp). One sub-group of lanes owns a whole front;
// sub-groups pack into warps. Kernel + launch + dispatch, all here.

#include "factorize/front_ops.cuh"
#include "factorize/mid_ablation.cuh"   // CLS_SMALL_ABLATION=1 → MAGMA/STRUMPACK-style ablation
...
// `mask` the sub-group's active-lane mask. Small fronts (fsz ≤ SG) keep all SG lanes busy
...
//
// At the leaves of the elimination tree the fronts are small (fsz ≲ 30, nc ≲ 8) but
...
// small fronts (fsz ≤ 16) keep all sub_group_size lanes busy and expose fronts_per_warp independent fronts' memory
...
static inline void launch_factor_small_t(int sub_group_size, int num_blocks, int threads_per_block, size_t shared_bytes, cudaStream_t stream,
                                         ...
                                         double pivot_shift)
{
    if (sub_group_size == 8)       launch_factor_small<FrontType, 8>(num_blocks, threads_per_block, shared_bytes, stream, b, level_size, B, front_area, plan, d_plc, frontB, sing, do_extend, static_pivoting, pivot_threshold, pivot_shift);
    else if (sub_group_size == 16) launch_factor_small<FrontType, 16>(num_blocks, threads_per_block, shared_bytes, stream, b, level_size, B, front_area, plan, d_plc, frontB, sing, do_extend, static_pivoting, pivot_threshold, pivot_shift);
    else               launch_factor_small<FrontType, 32>(num_blocks, threads_per_block, shared_bytes, stream, b, level_size, B, front_area, plan, d_plc, frontB, sing, do_extend, static_pivoting, pivot_threshold, pivot_shift);
}

// SMALL tier: sub-group-packed kernel. One sub-group of sub_group_size lanes owns one front; 32/sub_group_size fronts
// pack per warp, sub_group_size chosen by max_fsz so the dominant small fronts keep all sub_group_size lanes busy. The
