...
// Forward solve with within-panel pivot application (Phase Σ.8). Same as fwd_small_warp_b
// but reads pivot[k] = swap target at LU step k, and swaps the gathered RHS accordingly
// before the triangular solve. selinv must be off (no inverse used in this path).
template <typename FT>
__global__ void mf_fwd_small_warp_pp_b(int lbegin, int level_size, int B, int slab,
                                       const int* __restrict__ plcols,
                                       const int* __restrict__ front_off,
                                       const int* __restrict__ front_ptr,
                                       const int* __restrict__ ncols,
                                       const int* __restrict__ front_rows, const FT* frontB,
                                       FT* yB, long front_total, int n,
                                       const int* __restrict__ pivotsB,
                                       const int* __restrict__ pivot_offset, int total_pivots)
{
    extern __shared__ unsigned char fsm_pp_raw[];
    FT* slabs = reinterpret_cast<FT*>(fsm_pp_raw);
    const int warp_in_blk = threadIdx.x >> 5;
    const int wg = (blockIdx.x * blockDim.x + threadIdx.x) >> 5;
    const int lane = threadIdx.x & 31;
    if (wg >= level_size * B) return;
    ...
    // Gather RHS into sh_piv, then apply pivot swaps sequentially (lane 0).
    for (int k = lane; k < nc; k += 32) sh_piv[k] = y[fr[k]];
    __syncwarp();
    if (lane == 0) {
        for (int k = 0; k < nc; ++k) {
            const int pk = piv[k];
    ...
    __syncwarp();
    // Warp-parallel forward substitution. After pivot swap, sh_piv holds P*b for the
    // panel rows; we write the computed y_local back into BOTH sh_piv (for the CB update
    // below) AND y[fr[k]] (for ancestor panels' RHS gather). Note: writing to y[fr[k]]
    // is correct because fr[k] is the global row of the k-th LOCAL panel row, and we want
    // the kth output of L^-1 P b stored there (in permuted-row basis); back-solve reads
    // it in the same position so the column-pivoting-free U inverts cleanly.
    {
        FT part = FT(0), sk = FT(0);
        for (int k = 0; k < nc; ++k) {
    ...
    __syncwarp();
    // Write computed y back AFTER the warp sub completes (all sh_piv entries final).
    for (int k = lane; k < nc; k += 32) y[fr[k]] = sh_piv[k];
    __syncwarp();
    for (int i = nc + lane; i < fsz; i += 32) {
        FT upd = FT(0);
