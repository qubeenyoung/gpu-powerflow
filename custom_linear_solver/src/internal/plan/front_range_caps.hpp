...
#include "internal/types.hpp"  // kTensorCoreUcCap — keep the max_uc clamp in lock-step with the TF32 gate
...
struct FrontRangeCaps {
  ...
  int max_uc = 1;        // largest contribution-block dim, clamped to <=
                         // kTensorCoreUcCap (shared-tile bound)
...
inline FrontRangeCaps ScanFrontRange(const plan::MultifrontalPlan& plan,
  ...
  FrontRangeCaps m;
  for (int q = b; q < e; ++q) {
    // Derive per-panel front size, pivot columns, contribution dim.
    const int pp = h_plc[q];
    const int fsz = plan.h_front_ptr[pp + 1] - plan.h_front_ptr[pp];
    const int nc = plan.h_ncols[pp];
    const int uc = fsz - nc;

    // Accumulate range maxima (max_uc clamped to the TF32 shared-tile bound).
    if (fsz > m.max_fsz) m.max_fsz = fsz;
    if (uc > m.max_uc && uc <= kTensorCoreUcCap) m.max_uc = uc;
