...
// Front-size caps over a contiguous range of panels, for host-side dispatch sizing.
//
// scan_front_range() makes one pass over the panels in a plcols sub-range and returns the maximum
// front size and contribution-block / pivot-column dimensions in that range — the caps the factor
// and solve kernels need to size their shared layout and pick thread counts / tier. Shared by the
// factorize and solve dispatchers (replaces the near-duplicate scan loops they used to carry).
...
struct FrontRangeCaps {
    ...
    int max_uc = 1;        // largest contribution-block dim, clamped to <= CLS_TC_UC_CAP (shared-tile bound)
...
inline FrontRangeCaps scan_front_range(const plan::MultifrontalPlan& plan, const int* h_plc,
                                       int b, int e)
{
    FrontRangeCaps m;
    for (int q = b; q < e; ++q) {
