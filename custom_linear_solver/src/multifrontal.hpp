...
// Numeric precision modes. The solve reads the front the factor produced.
...
//   TC   – FP32 front + FP16 tensor-core (WMMA) trailing GEMM with FP32 accumulate.
...
struct State {
    ...
    float*  d_frontBf = nullptr;   // B * front_total FP32 front (FP32 / TC modes)
    double* d_yB  = nullptr;       // B * n FP64 solve working vector (FP64 mode)
    float*  d_yBf = nullptr;       // B * n FP32 solve working vector (FP32 / TC modes)
    int* d_sing = nullptr;
