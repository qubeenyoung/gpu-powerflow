...
static int compute_effective_panel_width(int n, int max_panel_width, bool float_front)
{
                                    ...
                                    int effective_panel_width, long total)
{
    ...
    fprintf(stderr, "[analyze] n=%d P=%d levels=%d panel_width=%d front_total(MB f32)=%.1f\n",
                                     ...
                                     bool emit_info)
{
    ...
    // When the factor/solve front is float (FP32 / TF32), allocate the float front arena.
                                      ...
                                      bool float_front, bool emit_info)
{
    ...
    const int effective_panel_width = compute_effective_panel_width(n, max_panel_width, float_front);
    sym::PanelPartition panels =
        forced_panels ? *forced_panels : sym::relaxed_panels(n, parent, colcount, effective_panel_width);
    ...
    if (emit_info) dump_front_distribution(symbolic_factor, plevel, n, P, num_plevels, effective_panel_width, total);
