...
__global__ void BuildAPosDevice(
  ...
  for (int q = Ap[j] + threadIdx.x; q < Ap[j + 1]; q += blockDim.x) {
    ...
    const int mx_local = (mx < first + ncols)
                             ? (mx - first)
                             : FrontLowerBound(front_rows, fbegin, fend, mx);
...
static bool DetermineAPosUnique(const int* d_a_pos, int nnz_a, long front_total,
  ...
  if (emit_info) {
    ...
    std::fprintf(
        ...
        fill_runs, fill_runs ? (double)fill_slots / (double)fill_runs : 0.0);
    std::fprintf(
        stderr, "[Analyze] front present slots=%ld (%.1f%% of front_total)\n",
        present, 100.0 * (double)present / std::max(1.0, (double)front_total));
...
static int ComputeEffectivePanelWidth(int n, int max_panel_width,
  ...
  if (effective_panel_width > 64) effective_panel_width = 64;
  return effective_panel_width;
}

// Per-panel front offset (in doubles) into the front arena = prefix sum of
// fsz², in panel-id (Postorder) order, which keeps each parent front adjacent
// to its children for extend-add. Returns false if the arena would exceed 1G
// doubles (8 GB).
static bool LayoutFrontArena(
...
static void DumpFrontDistribution(
  ...
  for (int p = 0; p < P; ++p) {
    const long fsz =
        symbolic_factor.front_ptr[p + 1] - symbolic_factor.front_ptr[p];
    int bck = 0;
    while (fsz > edges[bck]) ++bck;
    cnt[bck]++;
    sf2[bck] += fsz * fsz;
    sf3[bck] += (double)fsz * fsz * fsz;
    tot2 += fsz * fsz;
    tot3 += (double)fsz * fsz * fsz;
  }
  fprintf(
      stderr,
      "[Analyze] n=%d P=%d levels=%d panel_width=%d front_total(MB f32)=%.1f\n",
      n, P, num_plevels, effective_panel_width, total * 4.0 / 1e6);
  const int lo[NB] = {1, 17, 33, 49, 65, 97, 161};
  for (int bk = 0; bk < NB; ++bk)
    fprintf(stderr, "  fsz[%4d..%-9d] cnt=%-8ld f2%%=%5.1f f3%%=%5.1f\n",
            lo[bk], edges[bk] == (1 << 30) ? 999999 : edges[bk], cnt[bk],
            100.0 * sf2[bk] / std::max(1L, tot2),
            100.0 * sf3[bk] / std::max(1e-9, tot3));
...
// Spine + K-subtree partition. Spine = contiguous "cnt=1 chain" at the top of
// the panel Etree. Below the spine, each panel's subtree root is the highest
// ancestor whose parent is in the spine (or -1); panels sharing a root form one
// subtree. Roots are capped at kMaxSubtreeStreams (largest kept, the rest
// merged into one spillover subtree), then plcols is re-sorted so each level
// groups panels subtree-major and tier-major within each subtree (spine last).
static void PartitionSubtrees(
    MultifrontalPlan& plan,
    const symbolic::MultifrontalSymbolic& symbolic_factor,
    std::vector<int>& plcols, int P, int num_plevels, bool fp64,
    bool emit_info) {
  auto level_cnt = [&](int L) {
  ...
  ++spine_bot;  // now spine_bot = lowest L with cnt=1 in the chain (start of
                // spine)
  ...
  if (plan.num_subtrees > 0) {
    ...
    // For each level: bucket panels by subtree (spine=-1 last); each subtree
    // cell is then ordered tier-major so multistream dispatch can tier-split it
    // (see h_subtree_level_*).
    for (int L = 0; L < num_plevels; ++L) {
      ...
      for (int k = 0; k < plan.num_subtrees; ++k) {
        ...
        for (int t = 0; t < NT; ++t) {  // tier-major within the cell
          plan.h_subtree_level_tier_off[tier_base + t] =
              (int)bucketed.size() + lo;
          for (int q = lo; q < hi; ++q) {
        ...
        plan.h_subtree_level_tier_off[tier_base + NT] =
            (int)bucketed.size() + lo;
        plan.h_subtree_level_cnt[(long)k * num_plevels + L] = cnt;
      }
      // Then spine panels (subtree_of_panel == -1) at the end (cnt=1 chain; no
      // split).
      for (int q = lo; q < hi; ++q) {
        const int p = plcols[q];
...
// Tier-homogeneous dispatch order. Within each level, group panels by kernel
// tier (ClassifyFrontTier) so the single-stream factor walk launches one
// right-sized kernel per (level, tier) instead of promoting a whole mixed level
// to its largest front's tier.
static void BuildTierOrder(
    MultifrontalPlan& plan,
    const symbolic::MultifrontalSymbolic& symbolic_factor,
...
// Returns the map; sets plan.asm_total.
static std::vector<int> BuildAssemblyMap(
    MultifrontalPlan& plan,
    const symbolic::MultifrontalSymbolic& symbolic_factor, int P) {
  ...
  for (int p = 0; p < P; ++p) {
    ...
    for (int a = symbolic_factor.asm_ptr[p]; a < symbolic_factor.asm_ptr[p + 1];
         ++a)
...
// Lay out the single device arena (kArenaAlignmentBytes-aligned sub-arrays),
// allocate it, upload the host plan arrays, and build the assembly position map
// (a_pos) on device. Returns false if the a_pos kernel fails.
static bool AllocateAndUploadPlan(
    MultifrontalPlan& plan,
    const symbolic::MultifrontalSymbolic& symbolic_factor,
    const symbolic::PanelPartition& panels, const std::vector<int>& front_off,
  ...
  long off = 0;
  const long o_front = off;
  off = align_up(off + total * sizeof(double));
  const long o_foff = off;
  off = align_up(off + (long)(P + 1) * sizeof(int));
  const long o_fptr = off;
  off = align_up(off + (long)(P + 1) * sizeof(int));
  const long o_nc = off;
  off = align_up(off + (long)P * sizeof(int));
  const long o_plc = off;
  off = align_up(off + (long)P * sizeof(int));
  const long o_par = off;
  off = align_up(off + (long)P * sizeof(int));
  const long o_aptr = off;
  off = align_up(off + (long)(P + 1) * sizeof(int));
  const long o_aloc = off;
  off = align_up(off + (long)std::max(1, plan.asm_total) * sizeof(int));
  const long o_apos = off;
  off = align_up(off + (long)std::max(1, plan.nnz) * sizeof(int));
  const int front_store = symbolic_factor.front_ptr[P];
  plan.front_store = front_store;
  const long o_fr = off;
  off = align_up(off + (long)std::max(1, front_store) * sizeof(int));
  const long o_pf = off;
  off = align_up(off + (long)std::max(1, P) * sizeof(int));
  const long o_pof = off;
  off = align_up(off + (long)std::max(1, n) * sizeof(int));
  const long o_y = off;
  ...
  // When the factor/Solve front is float (FP32 / TF32), allocate the float
  // front arena.
  ...
  if (!plan.h_spine_panels.empty()) {
    ...
    cudaMemcpy(plan.d_spine_panels, plan.h_spine_panels.data(),
               plan.h_spine_panels.size() * sizeof(int),
  ...
  plan.a_pos_unique =
      DetermineAPosUnique(plan.d_a_pos, plan.nnz, total, emit_info);
  return true;
}
...
MultifrontalPlan AnalyzeMultifrontal(
    ...
    bool float_front, bool emit_info, bool dump_fronts_only) {
  ...
  const int effective_panel_width =
      ComputeEffectivePanelWidth(n, max_panel_width, float_front);
  sym::PanelPartition panels =
      forced_panels
          ? *forced_panels
          : sym::RelaxedPanels(n, parent, colcount, effective_panel_width);
  const sym::MultifrontalSymbolic symbolic_factor =
  ...
  {
    std::vector<int> next(plan.panel_level_ptr.begin(),
                          plan.panel_level_ptr.end());
  ...
  if (emit_info)
    DumpFrontDistribution(symbolic_factor, plevel, n, P, num_plevels,
                          effective_panel_width, symbolic_total);
  if (dump_fronts_only) {
    plan.front_total = symbolic_total;
    return plan;
  }
