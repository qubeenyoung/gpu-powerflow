...
//   max_panel_width              - panel cap hint (Analyze may override for
//   large n) float_front            - true if the factor/Solve front is float
//   (FP32/TC/TF32); controls
//                            whether float scratch arenas (d_frontf, d_yf) get
//                            allocated
//   forced_panels          - optional caller-supplied panel partition (else
//   built here) emit_info              - if true, print front-size and subtree
//   summary to stderr dump_fronts_only       - if true, stop after host
//   symbolic front/level metadata is built
