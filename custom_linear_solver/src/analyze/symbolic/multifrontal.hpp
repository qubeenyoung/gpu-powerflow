...
// relaxed panel it builds the DENSE FRONT row pattern, the panel elimination
// tree, and the EXTEND-ADD map that places each panel's contribution block (CB)
// into its parent front. All value-independent (depends only on the fill
// pattern + panel partition) -> built once in Analyze, reused every Factorize.
//
...
// After eliminating the ncols pivots, the trailing (front-ncols) rows form the
// CB, a dense block that is extend-added into the parent front via asm_idx.
struct MultifrontalSymbolic {
  int num_panels = 0;
  std::vector<int> panel_parent;  // panel id -> parent panel id (-1 = root)
  std::vector<int>
      front_ptr;  // panel id -> [start,end) in front_rows (size P+1)
  std::vector<int>
      front_rows;  // flattened sorted front row indices (pivots first)
  std::vector<int> asm_ptr;  // panel id -> [start,end) in asm_idx (size P+1)
  std::vector<int>
      asm_idx;  // for each CB row of P, its position in the parent
...
// RelaxedPanels(). asm_idx entries are >= 0 iff the multifrontal invariant
