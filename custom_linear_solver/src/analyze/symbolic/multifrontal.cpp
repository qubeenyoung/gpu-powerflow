...
namespace {
// Parallelize independent per-panel work (each panel's front-rows / asm_idx
// depends only on read-only L + already-final parent rows). 12-core ParFor,
// gated on panel count so small problems stay serial (thread overhead > gain).
template <typename Fn>
void ParPanels(int lo, int hi, Fn&& fn) {
...
// Build the multifrontal symbolic structure from the fill pattern + panel
// partition.
...
MultifrontalSymbolic ComputeMultifrontalSymbolic(int n,
  ...
  ParPanels(0, P, [&](int plo, int phi) {
    std::vector<int> mark(
        n, -1);  // per-thread (reused across this chunk's panels)
  ...
  ParPanels(0, P, [&](int plo, int phi) {
    for (int p = plo; p < phi; ++p) {
      std::copy(fr[p].begin(), fr[p].end(),
                mf.front_rows.begin() + mf.front_ptr[p]);
    }
  });
