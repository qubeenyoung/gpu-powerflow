...
// Postorder and column counts, and returns 2*nnz(L) - n (an L+U estimate used
// to compare candidate orderings without factorizing).
...
// arrays (no values). Used to score a candidate ordering via PredictedFill.
...
// Predicted L+U fill proxy for a CSC matrix reordered by perm (= PredictedFill
...
// sorted per column. Total nnz(Li) equals sum(ColumnCounts). This is the
// static pattern the M3 numeric factorization fills in.
