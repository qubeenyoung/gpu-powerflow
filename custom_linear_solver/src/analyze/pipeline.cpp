...
bool structural_matching_from_csc(int n, const std::vector<int>& col_ptr,
                                  const std::vector<int>& row_idx,
                            ...
                            PlanBuildResult& out)
{
    try {
        ...
        if (options.use_matching) {
            if (matrix::download_csc_structure(csc_device, csc_col_ptr, csc_row_idx) !=
                Status::kSuccess)
                return false;
            if (!structural_matching_from_csc(n, csc_col_ptr, csc_row_idx, row_to_col)) {
                std::fprintf(stderr, "[analyze] structural matching requested but no perfect matching exists\n");
                return false;
            }
        }
        ...
        std::vector<int> nd_perm(static_cast<std::size_t>(n), 0);
        {
            ...
            if (!reordering::metis_nd_from_graph(n, nd_xadj, nd_adjncy, nd_perm,
                                                 parallel_nd, metis_seed))
                return false;
        }
        out.perm = nd_perm;  // row order: new row -> original row
        out.iperm.assign(static_cast<std::size_t>(n), 0);  // column inverse: original col -> new col
        if (options.use_matching) {
        ...
        if (options.use_matching && d_row_iperm.upload(row_iperm) != Status::kSuccess)
            return false;
        matrix::DeviceCscPattern ordered_device;
        if (options.use_matching) {
            if (matrix::permute_csc_device_rc(csc_device, d_row_iperm.ptr, out.d_iperm.ptr,
        ...
        if (options.use_matching) {
            if (matrix::build_symmetric_graph_device(ordered_device, sym_col_ptr, sym_row_idx)
                != Status::kSuccess)
                return false;
        } else {
            permute_symmetric_pattern(n, metis_sym_col_ptr, metis_sym_row_idx, out.perm,
