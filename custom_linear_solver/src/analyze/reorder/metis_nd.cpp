...
int ParallelNdBaseThreshold(int n) {
  return n < 20000 ? CLS_PAR_ND_SMALL_BASE_THR : CLS_PAR_ND_LARGE_BASE_THR;
}

// Parallel nested dissection. METIS_NodeND is single-threaded; ND is
// recursively parallel (after a vertex separator, the two halves are
// independent). Find a separator with METIS, recurse on each half on separate
// threads, order the separator last. Same ND algorithm → fill ~= serial METIS,
// but uses all cores. Returns perm in METIS convention: perm[old_local] =
// new_local_position.
...
// Used at the root recursion level where only one thread is otherwise active;
// the inner ParFor self- limits to serial for small subgraphs, so deeper
// (already thread-saturated) levels keep the serial Induce().
...
void BaseNodend(const std::vector<idx_t>& xadj, const std::vector<idx_t>& adj,
  ...
  idx_t nv = n;
  // METIS_NodeND is read-only on the graph; BaseNodend is the recursion
  // terminal so xadj/adj aren't reused — pass them directly without copying.
  ...
  std::srand(seed);  // reseed per call -> each subgraph starts from a fixed RNG
                     // state (deterministic with the LD_PRELOAD thread-safe
                     // rand; no-op otherwise)
...
void ParNdRec(const std::vector<idx_t>& xadj, const std::vector<idx_t>& adj,
  ...
  // No graph copy. METIS_ComputeVertexSeparator treats the input graph as
  // read-only (it copies into its own internal graph_t), and Induce() below
  // reads the ORIGINAL xadj/adj — so pass them directly (const_cast only for
  // the C API signature). Removes a full-graph copy at every recursion level
  // (the root copy is on the sequential parND critical path).
  ...
  // Use the multi-core Induce on the few TOP levels (large subgraphs, few
  // active recursion threads), where the single-threaded Induce sits on the
  // critical path. The inner ParFor self-limits to serial below 32768, so
  // deeper/smaller subgraphs keep serial Induce and the oversubscription from
  // concurrent branches stays bounded.
  ...
  ParNdRec(x1, a1, p1, depth - 1, base_thr, seed,
           false);  // recursive calls go through METIS
  th.join();
  // perm convention is perm[new_position] = old_vertex (matches the caller's
  // permute_sym and METIS_NodeND's first output). p0[np]=old_local in part0 at
  // sub-newpos np.
  perm.assign(n, 0);
  for (int np = 0; np < n0; ++np) perm[np] = m0[p0[np]];  // part 0 -> [0, n0)
  for (int np = 0; np < n1; ++np)
    perm[n0 + np] = m1[p1[np]];  // part 1 -> [n0, n0+n1)
  int j = 0;
  for (int v = 0; v < n; ++v)
...
void InducePar(const std::vector<idx_t>& xadj, const std::vector<idx_t>& adj,
               ...
               std::vector<int>& map) {
  const int n = static_cast<int>(xadj.size()) - 1;
  // Local relabel: keep vertices with part[v]==which in ascending vertex order.
  // The scan that assigns local indices is serial (O(n), ~0.1ms) but the
  // count/fill passes are multi-core. Output is byte-identical to serial
  // Induce().
  std::vector<int> g2l(n, -1);
  map.clear();
  for (int v = 0; v < n; ++v)
...
bool RunNdOnGraph(int n, std::vector<idx_t>& xadj, std::vector<idx_t>& adjncy,
  ...
  if (parallel) {
    const int base_thr = ParallelNdBaseThreshold(n);
    std::vector<int> pp;
    ParNdRec(xadj, adjncy, pp, kParNdDepth, base_thr, seed);
...
bool MetisNdFromGraph(int n, std::vector<int>& xadj_in,
  ...
  // Adopt the prebuilt (GPU-produced) symmetric graph as idx_t. When idx_t ==
  // int the host arrays are moved in with no copy; otherwise widen
  // element-wise.
  ...
  return RunNdOnGraph(n, xadj, adjncy, perm, parallel, seed);
}
