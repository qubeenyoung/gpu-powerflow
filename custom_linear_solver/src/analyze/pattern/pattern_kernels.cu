...
// ============================================================================
// Analyze-phase device pattern kernels: device CSR -> CSC, fill-reducing
// permutation of a CSC pattern, and the symmetric (A+Aᵀ) adjacency graph METIS
// consumes. All run on the GPU so the only host transfers in Analyze are the
// small permutation arrays. source_pos[] threads each CSC entry back to its CSR
// slot so Factorize can scatter values without rebuilding the map.
// ============================================================================

namespace custom_linear_solver::matrix {
namespace {

Status CudaStatus(cudaError_t err) {
  return err == cudaSuccess ? Status::kSuccess : Status::kAnalysisFailed;
}

// Tally per-column nnz into shifted_counts[col+1] (an inclusive scan later
// makes col_ptr).
__global__ void CountCsrColumns(int nnz, const int* __restrict__ csr_col_idx,
                                int* __restrict__ shifted_counts) {
  const int p = blockIdx.x * blockDim.x + threadIdx.x;
  if (p < nnz) atomicAdd(&shifted_counts[csr_col_idx[p] + 1], 1);
}

// Scatter CSR entries into CSC slots (next_col = running write cursor per
// column); records the source CSR position of each CSC entry in source_pos.
__global__ void ScatterCsrToCsc(int n, const int* __restrict__ csr_row_ptr,
                                const int* __restrict__ csr_col_idx,
                                int* __restrict__ next_col,
                                int* __restrict__ csc_row_idx,
...
// Per-column size of the permuted matrix: new column iperm[old] keeps old
// column's nnz.
__global__ void CountPermutedColumns(int n, const int* __restrict__ col_ptr,
...
// Per-column size for a row/column permutation: the new column is
// d_col_iperm[old_col].
__global__ void CountPermutedColumnsRc(int n, const int* __restrict__ col_ptr,
...
// Copy old column old_col into new column iperm[old_col], remapping row indices
// through iperm and carrying source_pos along. One block per column.
__global__ void FillPermutedCsc(int n, const int* __restrict__ in_col_ptr,
  ...
  for (int offset = threadIdx.x; offset < in_end - in_begin;
       offset += blockDim.x) {
...
// Copy old column old_col into new column col_iperm[old_col], remapping rows
// through row_iperm. This supports static structural matching where row and
// column permutations differ.
__global__ void FillPermutedCscRc(int n, const int* __restrict__ in_col_ptr,
                                  const int* __restrict__ in_row_idx,
                                  const int* __restrict__ in_source_pos,
...
// Emit both directed edge keys (row*N+col and col*N+row) for every off-diagonal
// CSC entry; diagonal/invalid entries get a sentinel that sorts to the very
// end. One block per column. keys size = 2*nnz, indexed by 2*p / 2*p+1 (p =
// global CSC entry index).
__global__ void EmitEdgeKeys(int n, long long N,
...
// Decode sorted-unique edge keys into adjncy (= key % N) and tally per-source
// degree.
__global__ void DecodeEdges(long m, long long N,
...
//   In : device CSC pattern.  Out: host CSR-style xadj[n+1] / adjncy (deduped,
//   diagonal removed).
// Method: emit both directed edge keys per entry, sort+unique, decode key%N ->
// neighbor.
Status BuildSymmetricGraphDevice(const DeviceCscPattern& csc,
                                 std::vector<int>& xadj,
  ...
  EmitEdgeKeys<<<n, 128>>>(n, N, csc.col_ptr.ptr, csc.row_idx.ptr, d_keys);
  ...
  if (cudaMalloc(&d_adjncy, static_cast<std::size_t>(std::max(1L, m)) *
      ...
      cudaMalloc(&d_deg, (static_cast<std::size_t>(n) + 1) * sizeof(int)) !=
          cudaSuccess) {
    cudaFree(d_keys);
    cudaFree(d_adjncy);
    cudaFree(d_deg);
  ...
  if (m > 0) {
    ...
    DecodeEdges<<<static_cast<int>((m + threads - 1) / threads), threads>>>(
...
IntDeviceBuffer::~IntDeviceBuffer() { reset(); }
...
Status IntDeviceBuffer::allocate(std::size_t values) {
  ...
  if (cudaMalloc(reinterpret_cast<void**>(&ptr), values * sizeof(int)) !=
      cudaSuccess) {
    ptr = nullptr;
    count = 0;
    return Status::kAllocationFailed;
...
Status IntDeviceBuffer::upload(const std::vector<int>& values) {
  ...
  return CudaStatus(cudaMemcpy(ptr, values.data(), values.size() * sizeof(int),
                               cudaMemcpyHostToDevice));
}

// Convert a device CSR pattern to CSC (count -> scan -> scatter), recording
// source_pos.
//   In : device CSR row_ptr/col_idx.  Out: csc (col_ptr/row_idx/source_pos on
//   device).
Status BuildCscFromCsrDevice(int n, int nnz, const int* d_csr_row_ptr,
  ...
  if (st != Status::kSuccess) return st;
  if (cudaMemset(counts.ptr, 0,
                 (static_cast<std::size_t>(n) + 1) * sizeof(int)) !=
      cudaSuccess)
    return Status::kAnalysisFailed;

  constexpr int threads = 256;
  CountCsrColumns<<<(nnz + threads - 1) / threads, threads>>>(
  ...
  if (st != Status::kSuccess) return st;
  if (cudaMemcpy(next_col.ptr, csc.col_ptr.ptr,
                 (static_cast<std::size_t>(n) + 1) * sizeof(int),
                 cudaMemcpyDeviceToDevice) != cudaSuccess)
    return Status::kAnalysisFailed;

  ScatterCsrToCsc<<<(n + threads - 1) / threads, threads>>>(
      n, d_csr_row_ptr, d_csr_col_idx, next_col.ptr, csc.row_idx.ptr,
      csc.source_pos.ptr);
  return CudaStatus(cudaGetLastError());
}

// Apply the fill-reducing permutation to a device CSC pattern (count -> scan ->
// fill).
//   In : csc + d_iperm (old->new index).  Out: ordered CSC with source_pos
//   preserved.
Status PermuteCscDevice(const DeviceCscPattern& csc, const int* d_iperm,
                        DeviceCscPattern& ordered) {
  if (csc.n <= 0 || csc.nnz < 0 || csc.col_ptr.ptr == nullptr ||
      csc.row_idx.ptr == nullptr || csc.source_pos.ptr == nullptr ||
      d_iperm == nullptr)
    return Status::kInvalidValue;
  ...
  constexpr int threads = 256;
  CountPermutedColumns<<<(csc.n + threads - 1) / threads, threads>>>(
...
Status PermuteCscDeviceRc(const DeviceCscPattern& csc, const int* d_row_iperm,
  ...
  constexpr int threads = 256;
  CountPermutedColumnsRc<<<(csc.n + threads - 1) / threads, threads>>>(
...
Status DownloadCscStructure(const DeviceCscPattern& csc,
                            std::vector<int>& col_ptr,
                            std::vector<int>& row_idx) {
  if (csc.n <= 0 || csc.nnz < 0 || csc.col_ptr.ptr == nullptr ||
      csc.row_idx.ptr == nullptr)
    return Status::kInvalidValue;
  ...
  if (cudaMemcpy(col_ptr.data(), csc.col_ptr.ptr,
    ...
    return Status::kAnalysisFailed;
  if (csc.nnz > 0 && cudaMemcpy(row_idx.data(), csc.row_idx.ptr,
