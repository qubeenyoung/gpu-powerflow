...
> **코드에 baked-in** 되었다 — TF32 텐서코어 trailing(small blocked / big panel / large thin-K), fused trail+extend,
...
- **항상 baked-in**: no-pivot, CUDA graph, METIS-ND, **4-tier 결정적 라우팅**(small/small/big/large —
  ...
  모두 TF32 TC(Ozaki 시 FP32-band)를 탄다(big panel TC 는 occupancy 회복으로 헤드룸이 없어 scalar 보다
  느리지만 일관성 위해 감수; 빠른 scalar big 은 `--precision fp32`). 멀티스트림 서브트리, 동형 디스패치,
  **TF32 TC trailing**, **fused trail+extend**, **panel-resident 커널 = big 티어(배치)**(occupancy 회복),
  **register-blocked trailing**(`CLS_TRAIL_RB=4`), **sync-free U-solve / row-fused panel LU**.
...
| `SolverConfig.tier_split` | true | 4-tier 결정적 분리 디스패치. false = 디버그용(레벨 전체를 최대 front 의 커널로 병합) |
| `CLS_TF32_OZAKI_TC2` / `..._FIRST_ORDER` | OFF | TF32 Ozaki 정확도 회복(권장 ON). first-order = tail-tail 항 생략 |
| `CLS_MID_PANEL` (+`_MIN`/`_MED`/`_MED_BLK`) | 1 (on) | panel-resident big 커널 (big-tier front occupancy 회복). 게이트 튜닝용 |
...
> baked-in 되어 더 이상 토글이 아닌 것: TF32 TC trailing(small/big/large), fused trail+extend, 케이스 크기별
> panel width, **panel-resident big 커널 · register-blocked trailing · sync-free U-solve**. 음성으로 판명되어
> `deprecated/` 로 옮겨진 실험(gather assembly, deep-K amalgamation, custom GPU-ND, tiled-trailing, mid-fewsync/
> sysblk)은 [`03-optimization-notes/07-batch-factorize-structural-2026-06-13.md`](03-optimization-notes/07-batch-factorize-structural-2026-06-13.md)
> §negative 와 `deprecated/README.md` 참조.
