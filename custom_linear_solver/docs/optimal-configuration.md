...
- **항상 baked-in**: no-pivot, CUDA graph, METIS-ND, 3단(small/mid/big) 라우팅, 멀티스트림 서브트리,
  동형 디스패치, **TF32 TC trailing**, **fused trail+extend**, **panel-resident mid 커널**(big-mid front,
  occupancy 회복), **register-blocked trailing**(`CLS_TRAIL_RB=4`), **sync-free U-solve / row-fused panel LU**.
- **B=1 가속 레버**(opt-in): `--precision tf32` + Ozaki(B=1 latency-bound → TC 가 critical path 단축,
  ...
  그리고 measured best-of-k ordering(`CLS_ORDER_MEASURE_K`, factorize 재사용 시 −6~13%,
  [`03-optimization-notes/08-ordering-best-of-k-2026-06-13.md`](03-optimization-notes/08-ordering-best-of-k-2026-06-13.md)).
...
| `CLS_MID_PANEL` (+`_MIN`/`_MED`/`_MED_BLK`) | 1 (on) | panel-resident mid 커널 (big-mid front occupancy 회복). 게이트 튜닝용 |
| `CLS_TRAIL_RB` | 4 | register-blocked trailing 폭 {0=scalar,2,4,8} |
| `CLS_ORDER_K` / `CLS_ORDER_MEASURE_K` | 1 / 0 | best-of-k ND 선택. proxy(`_K`)는 anti-informative — 실익은 measured(`_MEASURE_K`, factorize 재사용 시) |
...
> panel width, **panel-resident mid 커널 · register-blocked trailing · sync-free U-solve**. 음성으로 판명되어
