...
> Canonical note: full benchmark table의 기준 문서다. 요약 결론과 권장 mode는
> [`01-final-report-2026-06-05.md`](01-final-report-2026-06-05.md)를 우선 참고한다.
...
### 2.1 FP64 의 위치
- 모든 case 에서 FP32 / TC 보다 **2-5× 느림**
- 큰 case (USA) 에서는 8× 까지 차이 (B=1)
- power-flow Newton-Raphson 같이 정확도 절대 안 필요한 use case 에서는 FP64 불필요
- 정확도 절대 필요한 경우 (refined IR pipeline 의 final step) 만 가치

### 2.2 FP32 vs TC 의 cross-over
...
**일관된 패턴**: B=1 ~ 4 에서 TC 가 항상 win (−5 ~ −17%). 큰 B 에서 cross-over → FP32 가 우세 (USA, ACTIVSg2000) 또는 tie.

### 2.3 TC win 의 크기 vs case 크기
...
큰 case 일수록 TC win 큼 (B 작을 때). USA 의 B=4 −17.5% 가 가장 큰 win — BIG-front 의 WMMA 가 가장 잘 fire.

ACTIVSg25k 는 anomaly — n=47k 인데 TC win 작음. 추정 원인: ACTIVSg25k 의 etree 가 USA 와 다른 구조 (BIG-front 가 적음).

### 2.4 큰 B 영역의 FP32 reclaim

ACTIVSg25k, USA 에서 B=64+ 에서 FP32 가 TC 이김. 이유:
- 큰 B 에서 batch 차원이 GPU 채움 → multistream 의 effect 약화
- WMMA 의 padding overhead 가 부각
- FP32 의 lean kernel set 이 long batched dispatch 에서 advantage
...
## 4. 절대 throughput — RTX 3090 의 power-flow 처리능력 (sys/sec)
...
> **B ≤ 16 작은 batch 에서는 TC, B ≥ 64 큰 batch 에서는 FP32 가 우세.**
> **USA 급 큰 grid 의 B=4 가 TC 의 sweet spot (−17.5%).**
> **FP64 는 모든 영역에서 2-5× 느림 — 정확도 절대 필요 없으면 사용 X.**

## 6. 환경 + 측정 조건

- 모든 측정 `--batch-only` (single-system path 우회), `cudaGraphLaunch` + `cudaEventSync` 기반 wall time
- 각 mode 동일 lever 적용: selinv OFF, multistream subtree dispatch, Σ.1 shared-staged trailing (FP32 path 도 포함)
...
| Σ.1 staged trailing | ON (Σ.14, FP32 도 포함) | mid kernel −20~30% |
