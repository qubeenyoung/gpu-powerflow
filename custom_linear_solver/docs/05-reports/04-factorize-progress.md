...
| `factor_small_single` | B=1 small tier | small-front 처리에서 batch indexing/modulo 제거 |
...
graph-node 프로파일(no-trailing): 잔여 wall은 `factor_small<float>` + `factor_mid<float>`가 지배.

- 25K B=256(5 calls 합): `factor_small` 53.48 ms > `factor_mid` 39.23 ms > `scatter_values_unique` 12.69 ms.
- 70K B=256(5 calls 합): `factor_small` 148.42 ms > `factor_mid` 100.53 ms > `factor_big` 68.60 ms > front arena `cudaMemsetAsync` 45.46 ms(~7.91 GB/call) > scatter 36.57 ms.
...
tier별: 13K는 거의 small-tier(no small extend −10.1%), 25K는 small+mid 분할(−11.9%/−9.8%), 70K는 small+big(−12.3%/−6.2%). tier 수치는 graph node 지속시간·stream overlap이 측정마다 달라 비-가산적이며, tier별 parent-update 재설계의 상한으로만 쓴다.
...
| `__ldg` metadata in `factor_small{,_single}` | full 0.053341 | revert |
...
`08`/`07` B=1 기각: `fsz 33..48` split, FP32 mid dynamic block 64/128/256, small tier 4/16 warp, reciprocal-multiply pivot, `ROWFUSED_NC_MAX=8`, `factor_mid __launch_bounds__(256,2)`, full tail-chain fusion(48 panels, 과직렬화), `CLS_B1_TINY`/`TIER_SPLIT`/`SMALL_COOP`/`SUBTREE_COOP`/`MIXED_SMALL`/`WARP_USOLVE`/`INIT_MAP`, TF32·TF32 WMMA, no-atomic small extend(잔차 깨짐). incomplete-factor+GMRES·deferred-spine은 solve로 작업 이동 → hard reset.
...
- Abdelfattah et al., "Batched One-Sided Factorizations of Tiny Matrices" (JOCS 2018): size ≤32는 vectorization·메모리 traffic·register blocking·concurrency가 핵심, ordinary GEMM 아님.
