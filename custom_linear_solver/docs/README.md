...
| **mid** | `fsz ≤ 64`(fp64) / `≤ 128`(float) (`kMidFrontMax`/`kMidFrontMaxFloat`) | `FactorMid` — whole-front shared-resident, 1 block/front | mid\|big 은 **정밀도 의존**. 배치에선 moderate front(65–128 float)를 shared-resident 단일 launch 로 합치는 게 3-launch big triple 보다 ~1–11% 빠름(launch 통합+shared 재사용+occupancy). fp64 는 128² double > 예산이라 64 |
| **big** | float >128 / fp64 >64 | `FactorBigPivot`→`Panels`→`Trail` — global-resident 3-launch 트리플, 한 front 를 여러 block 에 분산. TF32 trailing 은 텐서코어(`FactorBigTrailTf32`) | front 가 적고 클 때 GPU 를 채우고, shared-resident 불가한 거대 separator(FEM/circuit root)도 흡수 |
...
- `WholeFrontSharedMax` (159 float / 111 double) 는 **shared 통째 적재 절대 상한**. mid 경계(float 128)는 그보다 낮게
  잡았다(128 초과 시 occupancy 저하로 big multi-block 우세). tier 경계: small\|mid = 32, mid\|big = 64(fp64)/128(float).
