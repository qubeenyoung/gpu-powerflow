...
| **big** | `fsz > 64` | `FactorBigPivot`→`Panels`→`Trail` — global-resident 3-launch 트리플, 한 front 를 여러 block 에 분산(pivot+panel+trailing multi-block). TF32 trailing 은 텐서코어(`FactorBigTrailTf32`) | front 가 적고 클 때 GPU 를 채우고, shared-resident 불가한 거대 separator(FEM/circuit root)도 흡수 |
