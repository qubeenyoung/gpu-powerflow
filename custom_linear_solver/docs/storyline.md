...
각 기여는 ablation 토글에 매핑된다. 빌드/실행 설정은 [`optimal-configuration.md`](optimal-configuration.md),
세부 측정은 [`03-optimization-notes/`](03-optimization-notes/) 참조.
...
요약: big-kernel 시간의 **61% 가 panel-LU+U-solve barrier, 19.5% 가 trailing**
([`03-optimization-notes/01-kernel-engineering.md`](03-optimization-notes/01-kernel-engineering.md)).
...
- **배치 정적 multifrontal**: 사전 확정 symbolic plan(analyze-once / refactorize), B-시스템 uniform
  ...
  한다. 차별점이 아니라 **공통 바닥**.
- **No-pivot 정적 구조**: power-grid 대각우세로 부분 pivoting 없이 정확
  ([`02-design-analysis/02-no-pivoting-proof.md`](02-design-analysis/02-no-pivoting-proof.md)). 정적 파이프라인의 **전제**.
...
- **Kernel optimization** *(구 "execution shaping")*: 각 커널 내부의 on-chip staging·occupancy·
  ...
  (연구 기여의 크기를 정직하게 보려면 이 부분을 분리해서 봐야 한다 — §5. 측정 세부는
  [`03-optimization-notes/01-kernel-engineering.md`](03-optimization-notes/01-kernel-engineering.md).)
...
- **근거**: 임계값 `small≤32, mid≤128` HW 근거
  [`02-design-analysis/05-tier-thresholds.md`](02-design-analysis/05-tier-thresholds.md). tier 순기여는
  whole-level vs tiered ablation 으로 측정 가능(예상 양성).
...
- **실측**: 동형 → 70K/USA B=64/256 wall **−11~15%**
  ([`03-optimization-notes/01-kernel-engineering.md`](03-optimization-notes/01-kernel-engineering.md));
  멀티스트림 → case8387 B=64 **−22%**, USA B=64 **−14%**
  ([`04-benchmarks-profiling/04-multistream-impact.md`](04-benchmarks-profiling/04-multistream-impact.md)).

### B. Numeric specialization — trailing 을 텐서코어에 맞춘다

> 진단 전제: trailing 은 `M=N=uc≈140, K=nc≈20` 의 **thin-K GEMM** → naive 하게는 텐서코어가 안 먹힘
> (mma 기준 K=2.5 tile, staging 이 mma 압도). 그래서 이 축은 *TC 를 그냥 켜는 것이 아니라 먹히게
> 만드는 것*이다. 근거: [`02-design-analysis/04-gemm-fraction-tc-ceiling.md`](02-design-analysis/04-gemm-fraction-tc-ceiling.md),
...
- **문제**: 저-fill 케이스(8387)는 front 가 너무 작아(`nc≈1–2`) 텐서코어 tile(K=8)을 못 채운다 — tile
  효율 1.5–13%. 아무리 좋은 mma 도 K=1 일감엔 무용.
...
- **실측**: 8387/13K 가 raw ratio 상 TC 목표권 진입(8387 B64 **1.24× raw**). **단, ⚠️ 정직성 caveat**
  (notes 54/55, [`03-optimization-notes/03-tensor-core-investigation.md`](03-optimization-notes/03-tensor-core-investigation.md)):
  이 1.24× 는 **cap-inflation 아티팩트**(fp32 baseline 을 큰 panel-cap 으로 묶어 느리게, tf32 를 작은 cap
  으로 빠르게)이고, **best-vs-best 공정 비교 시 net 이득 ≈ 0**(≤10K 무가속, case3012wp 는 손해). 또한
  **fragile 4-플래그 번들** + `--serial-nd` + 고정 seed/cap 에서만 성립. → low-fill 정책은 large 정책과
  분리하고, 일반 권장에서 제외한다.
...
- **Column-owned U-solve** (`CLS_TF32_COLUMN_USOLVE`): 61% 병목(panel-LU+U-solve barrier)을 겨눠
...
| B2 Front coarsening | `TC_CLOSURE_*` off | 8387/13K TC 진입 여부 (net ≈ 0, §3B2 caveat) |
...
**추가 정정(notes 54/55)**: best-vs-best 통제 재현에서 ≤10K 는 무가속, 25K +5~7%, 70K/USA +10~16%(B=1
피크)로 *어디서도 1.2× 미달*이며, low-fill 번들은 net 이득 0. headline 1.2~1.3× 는 baseline cap inflation
산물이었다.
