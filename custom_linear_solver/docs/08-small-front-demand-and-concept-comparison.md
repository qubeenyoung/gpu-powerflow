# small-front 전용 솔버의 수요 + STRUMPACK vs custom 개념 비교

> **상태**: reference (2026-06-16 세션 종합)   **목적**: (1) small-front 특화 솔버에 시장/연구 수요가 있는가,
...
# PART 1 — small-front 전용 솔버의 수요가 있는가?

## 1.1 먼저, "small-front 행렬류"의 정의 (측정 기반)
거의 모든 희소행렬은 elimination tree의 *leaf*가 small라 **개수**로는 small front가 많다. 구분선은 개수가 아니라
**작업(트레일링 FLOP)이 작은 front에 머무느냐**다(§07 측정):

| 행렬 | 작업 in fsz≤64 | 작업 in fsz>160 | 최대 front | 판정 |
|---|---|---|---|---|
| power-flow 25k | **71%** | 0% | ~96 | small-front (작업까지) |
| 회로(scircuit) | 55% | 1.7% | ~1개 | small-front |
| 2D-FEM(parabolic) | 0.6% | **97%** | 수백 | big-front |
| 3D-FEM(cant) | 0.2% | **99%** | 수백 | big-front |

→ **전력조류·회로는 *작업까지* small-front에 갇힌 드문 클래스**다. 전력망이 near-planar + 저차수라 separator가
작게 유지되기 때문(n이 커져도 최대 front가 96→160으로만 자람). 2D/3D PDE는 separator가 √n / n^(2/3)로 커져
작업이 큰 front로 이동한다.

## 1.2 이 클래스에 *수요*가 있는가 — 응용
핵심은 **small-front × 같은 패턴 다수 시스템(batched/반복)**의 교집합. 이게 본질적으로 나오는 워크로드:
...
## 1.3 정직한 수요 평가
- **공급 공백은 실재한다**: GPU 솔버 중 *batched-small-front 특화*는 없다. cuDSS는 범용(UBATCH는 있으나 small-front
  특화 아님), STRUMPACK은 범용 멀티프론탈(small-front에서 §07처럼 15× 느림). 그 틈이 곧 수요.
...
- **결론**: "범용 솔버 수요"는 없다(cuDSS가 채움). "**batched small-front 특화 가속**"의 수요는 **전력계통에서
...
| | STRUMPACK | custom |
|---|---|---|
| 한 문장 | "**어떤** 희소행렬이든 nested dissection으로 분해해, 레벨별로 *벤더 배치 BLAS(vbatched)*로 처리하는 **범용** GPU 멀티프론탈." | "front보다 작은 **sub-group을 병렬 원자**로 삼아 *packing과 full-pipeline fusion을 동시에* 하는 **small-front 특화** GPU 멀티프론탈." |
...
- 결과: 일반성·안정성↑, 그러나 small-front에서 **op별 분리 + host 오케스트레이션 + one-block-per-front**로 GPU underfill.
...
성립한다(=small-front). front가 크면(아래 §3.3-⑦) custom도 타일로 쪼개야 하고, 아주 크면 STRUMPACK의 큰-GEMM
vbatched가 유리해진다.
...
custom = **"front를 sub-group으로 쪼개 packing+fusion을 동시에 하는 small-front 특화"**(작은 front·배치에 강함).
*입도(operation vs sub-front)*가 두 개념의 본질적 분기점이고, 그 분기가 곧 각자의 강점 영역(큰 3D vs small-front)을 결정한다.
...
- **small-front에서 손해**: ① op별 분리 → global 왕복·배리어, ② 깊은 트리(전력망 ~78레벨) × 5커널 = 수백 launch,
...
**① sub-group packing + full-pipeline fusion (핵심, small tier)**
...
- *왜*: STRUMPACK은 packing(vbatched)이나 fusion(native 커널) 중 **하나만** 한다(입도가 배타적: op-batched는
  ...
  small-front에선 **sub-group 입도**로 둘을 동시에 → SM을 채우면서(packing) 데이터를 on-chip 유지(fusion).
...
- *효과(FP64)*: parabolic factor **759→52ms(14×)**, cant 59→50ms. cuDSS 대비 parabolic **1.5×**, cant 1.26×.
  → 큰 front까지 처리 가능해져 **"small-front 클래스"의 경계가 2D-FEM·일부 3D로 확장**(일반화의 핵심).
