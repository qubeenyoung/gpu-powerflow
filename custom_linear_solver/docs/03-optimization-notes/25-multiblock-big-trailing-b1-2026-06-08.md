...
## proper TC multi-block 시도 — **negative result (2026-06-08), 코드 제거**

scalar-MB 라우팅은 routed level 에서 TC 를 포기하므로, TC trailing 자체를 M-strip 으로 분할
(`factor_big_trailing_tf32_mb<SPB>`: 블록당 L strip subset + **full U staging**, blockIdx.z = strip tile)
하면 TC 속도 + fanout 을 둘 다 얻으리라 가정하고 구현·측정.

**결과: 실패.** USA tf32 B=1 factor — TC-MB **2.27ms** vs scalar-MB **1.90ms** vs fused-TC 2.22ms.
즉 TC-MB 가 scalar-MB 보다 19% 느리고 fused 보다도 느림. 메커니즘:
- severe underfill 은 **occupancy-bound** → 중요한 건 fanout 의 granularity 다.
- scalar-MB: element-tile (USA ~24 tile/front), **staging 0** (global 직접 read).
- TC-MB: M-strip tile (SPB=2 → 7 tile/front, 굵음) + **block 마다 full U(~28KB) 중복 staging**.
- multi-block GEMM 은 L-row / U-col 재read 가 구조적 (A-panel × B-panel) → staging 중복 불가피.
  occupancy-bound 에선 TC 의 빠른 연산이 무의미하고, 중복 staging 오버헤드가 fused 보다도 느리게 만듦.

**교훈**: severe-underfill(B=1 deep) 의 trailing 은 *연산속도* 가 아니라 *fanout + 오버헤드* 게임.
가장 잘게 펼치고 staging 없는 scalar element-tile (scalar-MB) 이 최적. TC 분할은 dead-end.
코드 제거, scalar-MB 라우팅 유지.
