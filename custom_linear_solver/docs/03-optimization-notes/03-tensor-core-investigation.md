...
- **tiny-front 지배 low-fill case(8387/13K)**: front 의 97% 가 `fsz<=16`, K=1/2 → 어떤 local
...
|---|---|---|---|
| upper-bound (36) | small-tier·extend-add 제거 timing 천장 | small제거 B256 ~1.12×; small+extend제거 B256 ~1.16× | 두 wall 제거해도 B256 1.2× 미달; tile eff ~13% |
| many-front packed TC (37) | 독립 tiny front 들을 1 MMA 타일에 block-diagonal pack | `small<=16` packed eff **6.5%**, useful share 21~22% | `(6,2,4)`·`(4,2,2)` 가 N차원 미충전; 4×~8× peak × 6.5% = 0.26~0.52× FP32 |
...
| panel-chain amalgamation (46) | parent-child 체인 패널 병합(cap32) | best paired 13K seed99/cap31 **1.042×**, 8387 0.941× | 패널 수 −1~2% 만, 양 case 97% tiny 잔존 |
| sibling+chain combo (47) | sibling 후 chain, 가장 강한 conservative 병합 | 8387 0.982~1.009×, 13K 0.976~1.028× | cap32 후도 >96% tiny; cap40/48 은 FP32 relres 발산(invalid) |
...
- **SuperLU_DIST batched** ([10.1177/10943420241268200](https://doi.org/10.1177/10943420241268200)):
  ...
  로 취급할 근거.
- **cuDSS blog**: batching 은 개별 시스템이 GPU 를 못 채울 때 enabler; tiny matrix 는 hybrid/host.
