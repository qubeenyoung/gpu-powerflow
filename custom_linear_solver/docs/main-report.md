...
| factorize(4-tier) | `src/factorize/{tiny,small,big,large}.cuh`, `factorize.cu` |
...
| front 를 **shared 단일 커널로 factor+update 융합** (tiny) | **STRUMPACK native** ("single NT×NT block per front", NT∈{8,16,24,32}) | Ghysels & Synk |
...
| | packing(SM 채움) | full-front fusion(on-chip) | tiny front 결과 |
|---|---|---|---|
...
| **우리 `factor_tiny`** | **O** | **O** | **occupancy 30–59 %, lane 45–70 %** |
...
- **STRUMPACK 융합커널은 front-monolithic 입도다.** F11/F12/F22 를 한 블록 shared 에 깔고 통째로 도는 구조라
  *front 하나에 블록 하나*가 강제된다. tiny front 면 블록이 거의 빈다. 융합이 monolithic 이라 **packing 불가**.
...
(`src/factorize/tiny.cuh`). 그러면 한 워프에 **32/SG 개 front 가 packing**되고(SM 채움), 각 front 는 자기
...
이 *동시* 구조 — "tiny front 를 packing 한 채로 각 front 의 full 파이프라인을 on-chip 융합" — 은 조사한 어느
...
**한 줄 판정**: *"전력조류 tiny-front 멀티프론탈에서, packing 과 full-front fusion 의 입도 배타성을 sub-group
분해로 해소해 둘을 한 커널에서 동시에 달성"* — 이 구조가 survey 범위 내 처음이고 측정된 16–66× 우위의 기전이다.

---

## 4. 4-tier 구현 — 그 원리를 front 크기 전체로 확장

§3 의 핵심(packing+fusion 동시)은 tiny 티어에 산다. 더 큰 front 로 가면 같은 원리(on-chip 상주를 최대화하며
점유를 지킴)를 크기에 맞춰 변형한다. front 가 어느 커널을 타는지는 **크기(와 정밀도)만의 결정적 함수**다 —
실행 시 점유·옵션에 따라 흔들리지 않는다.

| tier | front 크기 | shared 전략 | 경계의 근거 |
|---|---|---|---|
