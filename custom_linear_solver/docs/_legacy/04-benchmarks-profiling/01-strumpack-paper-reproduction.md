...
> *"general GPU multifrontal libraries are engineered for matrices with **large dense fronts** ...; power-grid Jacobians have **no large fronts**, so those libraries run at a small fraction of peak"*

— 같은 multifrontal 패밀리 안에서 STRUMPACK은 large-front, custom은 small-front를 타깃. 본 실험은 그 분할을 직접 확인한 측정이다.
