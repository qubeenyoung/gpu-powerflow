...
mid tier 가 가장 늦게 포화(작은 work, latency dominant), big tier 가 가장 빨리 포화하면서 wall delta 가
...
`factor_mid` 은 이미 `__syncwarp` + warp-per-front 라 barrier 미관측. T4 는 mid/big 의 FP32 path 대상.
...
thread 당 nc serial FMA 가 sync 절감 압도 → USA +13~20% 회귀라 gate 필수. factor_mid + cp.async 는 USA B=1
...
(uc>32 가 11.7%) — high-conflict reduction 아닌 small traffic. 25K/USA 는 `>256` update 가 56–64% 라
...
- **subwarp small tiling** (24): 1 warp 를 SG-lane sub-group(fsz≤8→8, ≤16→16)으로 쪼개 32/SG front 동시 처리,
  ...
  **채택**(default-on). 교훈: mid tier 의 진짜 lever 는 resident warp 수가 아니라 per-warp lane 효율.
...
| factor_mid cp.async | **OFF** | USA B=1 +12% 회귀 | (미시핑) |
...
| subwarp small tiling (SG-lane) | **ON** | case8387 FP64 B=64 F+S −14~16% | `CLS_MID_SG32_ONLY` |
