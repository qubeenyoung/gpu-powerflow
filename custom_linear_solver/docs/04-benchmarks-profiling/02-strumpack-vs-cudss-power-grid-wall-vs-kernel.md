...
세부 측정 데이터는 `docs/04-benchmarks-profiling/01-strumpack-paper-table2-reproduction.md` (paper 행렬 단독) 와 `docs/01-orientation/03-lineage-strumpack-not-the-baseline.md` (베이스라인 lineage) 참고. 본 문서는 그 둘을 한 줄기로 잇는 분석 logic 위주.
...
5회 평균 (`docs/04-benchmarks-profiling/01-strumpack-paper-table2-reproduction.md` §3.1):
...
이 차이는 본 저장소 `docs/01-orientation/02-related-work-and-novelty.md` §2 가 외부 출처 (Spatula, MICRO'23) 와 함께 다음으로 정량화:
...
따라서 custom의 W ≈ K (97%, `docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md`), STRUMPACK/cuDSS는 W ≫ K. 두 measurement 모두 의미 있음 — W는 사용자 NR loop가 보는 per-call latency, K는 순수 GPU 효율.
...
- `custom_linear_solver`는 STRUMPACK 코드 베이스가 아니다 (`docs/01-orientation/03-lineage-strumpack-not-the-baseline.md` §1)
...
- 본 저장소 측정 (`docs/01-orientation/02-related-work-and-novelty.md` §2, `docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md`):
...
| L# | 개입 | 코드 위치 | 측정 효과 |
|---|---|---|---|
| **L1** | End-to-end device-resident API: set_data/set_rhs/set_solution 디바이스 포인터, solve도 device 솔브 그래프 replay | `src/solve/multifrontal.cu`, `docs/01-orientation/01-api-and-build-design.md` | 본 측정 case_ACTIVSg25k solve: 0.66 ms (custom) vs 9.27 ms STRUMPACK (CPU 경고) |
| **L2** | **Warp-per-front 전용 커널** (`mf_factor_small_warp_b`): fsz≤32 레벨은 1 warp/front, per-warp shared, `__syncwarp()` 만 | `src/batched/factor_small.cuh` | 본 저장소 `docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md`: dominant bottom level 2.47→1.12 ms, compute-bound 76% |
...
| **L9** | **factor/solve CUDA Graph capture + replay**: analyze 시점 한 번 캡처, 매 NR iter에서 replay → launch overhead 제거 | `docs/01-orientation/01-api-and-build-design.md` | "kernel time = factor/solve wall-clock의 ~97%" |
...
- L5 (batched) + L6 (FP32-native) = batched factor+solve 추가 −25~28% (`docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md`)
...
| 한계 | (a) 가정이 깨진 매트릭스(large front, mid-failure ACTIVSg70k 이상) 에는 무력 (b) head-to-head vs 외부 batched 솔버 (Wang/Fraunhofer, Zhou) 미실행 (`docs/01-orientation/02-related-work-and-novelty.md` §5) |
...
본 저장소:
- `docs/04-benchmarks-profiling/01-strumpack-paper-table2-reproduction.md` — paper Table 2 재현 측정 단독
- `docs/01-orientation/03-lineage-strumpack-not-the-baseline.md` — lineage, STRUMPACK ≠ 코드 베이스 증거
- `docs/01-orientation/02-related-work-and-novelty.md` — 외부 솔버 landscape + 인용 + novelty 자체 평가
- `docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md` — FP32 batched 측정
- `docs/03-optimization-notes/03-analyze-phase-optimization.md`, `docs/03-optimization-notes/02-factor-solve-analyze-optimization.md` — 단계별 최적화
- `docs/03-optimization-notes/04-tensor-core-factor-design.md` — TC32 negative result
- `docs/03-optimization-notes/05-mysolver-warm-cache-port-plan.md` — single-case path vs cuDSS
