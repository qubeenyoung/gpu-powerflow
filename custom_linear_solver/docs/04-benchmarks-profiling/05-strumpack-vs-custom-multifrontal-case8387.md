...
`docs/01-orientation/03-lineage-strumpack-not-the-baseline.md` 에서 정리한 대로 STRUMPACK과 `custom_linear_solver`는 **둘 다 multifrontal LU** 패밀리다. 둘 다 METIS nested dissection, elimination tree, supernode amalgamation, front + extend-add를 사용한다. 같은 알고리즘 패밀리인데 전력망 야코비안 case8387pegase (n=14908, nnz=110572) 에서 NR steady-state 성능은:
...
(cuDSS 와의 비교는 본 문서 범위 밖. `docs/04-benchmarks-profiling/04-nsys-three-solvers-nr-loop-profile.md` 에 별도.)
...
이건 본 저장소 `docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md` 의 3-tier 커널 라우팅과 정확히 같은 기준이다.
...
STRUMPACK 논문 (Claus, Ghysels, Boukaram, Li, IJHPCA 2025) 의 Table 2 가 자체 평가에 사용한 SuiteSparse 행렬 (`docs/04-benchmarks-profiling/01-strumpack-paper-table2-reproduction.md` §2 와 `docs/04-benchmarks-profiling/02-strumpack-vs-cudss-power-grid-wall-vs-kernel.md` §1):
...
- case8387pegase 한 매트릭스. 다른 power-grid (1k, 3k, 25k bus) 도 같은 front 분포 (`docs/01-orientation/02-related-work-and-novelty.md` §2 가 보고) — 정성적으로 일반화 가능.
...
- `docs/01-orientation/03-lineage-strumpack-not-the-baseline.md` — STRUMPACK ≠ 코드 베이스, lineage 정정
- `docs/04-benchmarks-profiling/01-strumpack-paper-table2-reproduction.md` — STRUMPACK paper 행렬 재현 결과
- `docs/04-benchmarks-profiling/02-strumpack-vs-cudss-power-grid-wall-vs-kernel.md` — 전력망 위 wall-clock + kernel-only 측정
- `docs/04-benchmarks-profiling/04-nsys-three-solvers-nr-loop-profile.md` — STRUMPACK vs cuDSS vs custom NR 프로파일 (case_ACTIVSg25k)
- `docs/04-benchmarks-profiling/03-nsys-strumpack-nr-loop-profile.md` — STRUMPACK nsys 상세 (case_ACTIVSg25k)
- `docs/03-optimization-notes/01-fp32-batched-kernel-optimization.md` — front 크기별 커널 라우팅 설계
- `docs/01-orientation/02-related-work-and-novelty.md` — 외부 출처 (Spatula MICRO'23) 인용 + novelty 자체 평가
