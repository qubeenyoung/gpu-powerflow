...
`docs/lineage-strumpack-not-the-baseline.md` 에서 정리한 대로 STRUMPACK과 `custom_linear_solver`는 **둘 다 multifrontal LU** 패밀리다. 둘 다 METIS nested dissection, elimination tree, supernode amalgamation, front + extend-add를 사용한다. 같은 알고리즘 패밀리인데 전력망 야코비안 case8387pegase (n=14908, nnz=110572) 에서 NR steady-state 성능은:
...
(cuDSS 와의 비교는 본 문서 범위 밖. `docs/nsys-three-solvers-nr-loop-profile.md` 에 별도.)
...
이건 본 저장소 `docs/fp32-batched-kernel-optimization.md` 의 3-tier 커널 라우팅과 정확히 같은 기준이다.
...
STRUMPACK 논문 (Claus, Ghysels, Boukaram, Li, IJHPCA 2025) 의 Table 2 가 자체 평가에 사용한 SuiteSparse 행렬 (`docs/strumpack-paper-table2-reproduction.md` §2 와 `docs/strumpack-vs-cudss-power-grid-wall-vs-kernel.md` §1):
...
- case8387pegase 한 매트릭스. 다른 power-grid (1k, 3k, 25k bus) 도 같은 front 분포 (`docs/related-work-and-novelty.md` §2 가 보고) — 정성적으로 일반화 가능.
...
- `docs/lineage-strumpack-not-the-baseline.md` — STRUMPACK ≠ 코드 베이스, lineage 정정
- `docs/strumpack-paper-table2-reproduction.md` — STRUMPACK paper 행렬 재현 결과
- `docs/strumpack-vs-cudss-power-grid-wall-vs-kernel.md` — 전력망 위 wall-clock + kernel-only 측정
- `docs/nsys-three-solvers-nr-loop-profile.md` — STRUMPACK vs cuDSS vs custom NR 프로파일 (case_ACTIVSg25k)
- `docs/nsys-strumpack-nr-loop-profile.md` — STRUMPACK nsys 상세 (case_ACTIVSg25k)
- `docs/fp32-batched-kernel-optimization.md` — front 크기별 커널 라우팅 설계
- `docs/related-work-and-novelty.md` — 외부 출처 (Spatula MICRO'23) 인용 + novelty 자체 평가
