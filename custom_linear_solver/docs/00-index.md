...
4. [Benchmark vs cuDSS 2026-06-07](05-reports/04-bench-vs-cudss-2026-06-07.md)
   bus-count > 1K MATPOWER NR Jacobians (case1197~case_SyntheticUSA) 위에서 cuDSS fp32 vs custom {fp32, fp16, tf32} × B∈{1, 4, 32, 64, 256} sweep. case8387 B=1 fp32 의 subtree 멀티스트림 ON/OFF nsys 프로파일 동봉. custom/tf32 가 B=256 에서 case 작을수록 큰 speedup (case1197 44.7×, case_SyntheticUSA 5.8×). NVTX timeline 으로 multi-stream overlap 시각 확인.
