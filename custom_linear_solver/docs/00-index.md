...
54. [Cleanup 중단 + front-distribution 스케줄링 정책 레이어 계획 2026-06-10](03-optimization-notes/54-cleanup-paused-and-scheduling-policy-plan-2026-06-10.md)
    소스 cleanup(3-정밀도 bake-in) 중단 status 로그. 이유: **케이스별 최적 알고리즘이 갈려** 입력 크기 n 이 아니라 **post-symbolic front 분포**로 결정해야 함 → 별도 정책 레이어 필요, bake 시기상조. 이번 세션 발견: exp#3 TC mma 순기여 ~38%(+6~9%), note 50 "8387 1.24×"는 cap30 ratio artifact(자연 cap 진짜 1.12×), low-fill 은 깨지기 쉬운 4-플래그 번들(작은 케이스 한정), column-U-solve ≈0%. 다음 방향: `policy(front_features, B, precision) → config` 스케줄링 레이어(설계만, 미실행 — 78 케이스 NR 데이터 생성 필요).

55. [fp32 vs TC 재현 + fused C-drain (fp16/tf32) 검증 2026-06-10](03-optimization-notes/55-repro-fp32-vs-tc-and-fused-cdrain-2026-06-10.md)
    대표 케이스(1K~3K/3K~6K/6K~10K/25K/70K/USA) × B{1,16,64,256} fp32 vs tf32 재현. **best-vs-best 통제(둘 다 codex-large, 각자 최적 cap)**: ≤10K 무가속(0.94~1.03×), 25K +5~7%, 70K/USA +10~16%(B=1 피크), **어디서도 1.2× 미달**. Codex 1.2~1.3× 헤드라인은 **baseline cap inflation** 산물(note 54 self-correction ~1.1× 확증). low-fill 번들 <10K **net 이득 0**(case3012wp 손해). solve TC 무관 + wall 30~46% → factor+solve 1.1× 천장. **fused trail+extend(C 왕복 제거)**: fp16/tf32 둘 다 **B=1만 이득**(fp16 70K B1 +33%, tf32 USA B1 +8%), **배치 무효~−10%**(overlap에 숨겨짐 + 흩어진 atomic 손해). tf32 fused 정확도 통과(note 53 correctness 버그 미재현)하나 **register spill(STACK:24, fp16은 0)** — fp16 mma 가 A 2-reg 라 여유. `CLS_FUSE_TF32_TRAIL_EXTEND` 토글(기본 OFF) 추가. 배치 글로벌 절감은 fused 아니라 **fp16 *저장*(half)** 이라야 함(미구현).
