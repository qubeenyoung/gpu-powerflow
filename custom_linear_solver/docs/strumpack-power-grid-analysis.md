...
세부 측정 데이터는 `docs/strumpack-reproduction-report.md` (paper 행렬 단독) 와 `docs/baseline-vs-strumpack.md` (베이스라인 lineage) 참고. 본 문서는 그 둘을 한 줄기로 잇는 분석 logic 위주.
...
5회 평균 (`docs/strumpack-reproduction-report.md` §3.1):
...
이 차이는 본 저장소 `docs/related-work-and-contribution.md` §2 가 외부 출처 (Spatula, MICRO'23) 와 함께 다음으로 정량화:
...
따라서 custom의 W ≈ K (97%, `docs/fp32-batched-factor-solve-optimization.md`), STRUMPACK/cuDSS는 W ≫ K. 두 measurement 모두 의미 있음 — W는 사용자 NR loop가 보는 per-call latency, K는 순수 GPU 효율.
...
- `custom_linear_solver`는 STRUMPACK 코드 베이스가 아니다 (`docs/baseline-vs-strumpack.md` §1)
...
- 본 저장소 측정 (`docs/related-work-and-contribution.md` §2, `docs/fp32-batched-factor-solve-optimization.md`):
...
| **L2** | **Warp-per-front 전용 커널** (`mf_factor_small_warp_b`): fsz≤32 레벨은 1 warp/front, per-warp shared, `__syncwarp()` 만 | `src/batched/factor_small.cuh` | 본 저장소 `docs/fp32-batched-factor-solve-optimization.md`: dominant bottom level 2.47→1.12 ms, compute-bound 76% |
...
- L5 (batched) + L6 (FP32-native) = batched factor+solve 추가 −25~28% (`docs/fp32-batched-factor-solve-optimization.md`)
...
| 한계 | (a) 가정이 깨진 매트릭스(large front, mid-failure ACTIVSg70k 이상) 에는 무력 (b) head-to-head vs 외부 batched 솔버 (Wang/Fraunhofer, Zhou) 미실행 (`docs/related-work-and-contribution.md` §5) |
...
본 저장소:
- `docs/strumpack-reproduction-report.md` — paper Table 2 재현 측정 단독
- `docs/baseline-vs-strumpack.md` — lineage, STRUMPACK ≠ 코드 베이스 증거
- `docs/related-work-and-contribution.md` — 외부 솔버 landscape + 인용 + novelty 자체 평가
- `docs/fp32-batched-factor-solve-optimization.md` — FP32 batched 측정
- `docs/analyze-bottleneck-and-optimization.md`, `docs/fsa-optimization-report.md` — 단계별 최적화
- `docs/tensor-core-factorize-design.md` — TC32 negative result
- `docs/warm-cache-stack-port-expectations.md` — single-case path vs cuDSS
