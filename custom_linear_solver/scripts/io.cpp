...
CsrMatrix read_matrix_market_csr(const std::filesystem::path& path)
{
    ...
    if (!input) {
        throw std::runtime_error("failed to open Matrix Market matrix: " + path.string());
    }
    ...
    if (info.format != "coordinate") {
        throw std::runtime_error("expected coordinate Matrix Market matrix: " + path.string());
    }
    if (info.field != "real" && info.field != "integer" && info.field != "pattern" &&
        info.field != "complex") {
        throw std::runtime_error("unsupported Matrix Market matrix field: " + info.field);
    }
    ...
    if (!shape || rows64 <= 0 || cols64 <= 0 || declared64 < 0) {
        throw std::runtime_error("invalid Matrix Market matrix shape: " + path.string());
    }
...
DenseVector read_matrix_market_vector(const std::filesystem::path& path)
{
    ...
    if (info.format != "array") {
        throw std::runtime_error("expected array Matrix Market vector: " + path.string());
    }
    ...
    if (info.symmetry != "general") {
        throw std::runtime_error("Matrix Market vector must be general: " + path.string());
    }
    ...
    if (!shape || rows64 <= 0 || cols64 <= 0) {
        throw std::runtime_error("invalid Matrix Market vector shape: " + path.string());
    }
                                ...
                                const std::vector<double>& values)
{
    ...
    if (!output) {
        throw std::runtime_error("failed to open Matrix Market vector for writing: " +
                                 path.string());
    }
